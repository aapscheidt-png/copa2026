...
const ESPN_CACHE_KEY_V22="copa2026_data_v10";
...
function manualLiveV10(m){
  const x = LIVE_MATCHES[matchKey(m.h,m.a)];
  if(!x) return null;
  const clk = liveClockV10(x.startISO);
  return {
    hs:x.hs,
    as:x.as,
    hasScore:true,
...
function safeRunV10(fn, ms=3500){
  return Promise.race([
...
// V10 - Base disciplinar local do torneio.
...
function allTournamentTeamsV10(){
  const set=new Set();
...
function cardPlayerNameV10(c){return c.name||c.player||c.athlete||c.displayName||c.text||"Jogador não informado";}
function cardMinuteV10(c){return c.minute||c.time||c.clock||"?";}
...
function renderStats(){
  ...
  const teams={};
  allTournamentTeamsV10().forEach(t=>teams[t]={nm:t,j:0,v:0,e:0,d:0,gp:0,gc:0,sg:0,pts:0,cs:0,yc:0,rc:0,totalCards:0,agg:false});
  ...
  F.filter(m=>mSt(m)!=="upcoming").forEach(m=>{
    ["home","away"].forEach(side=>{
      ...
      cards.forEach(c=>{
        const type=cardType(c), name=cardPlayerNameV10(c), minute=cardMinuteV10(c);
  ...
  return`<div class="stats-version">✓ V10"kpi-grid">
...
<table class="info-tbl">
  ...
  <tr><td>Versão</td><td style="color:var(--gold)">V10</td></tr>
</table></div>`;
}
...
function renderBrasil(){
  ...
  return `<div class="pro-badge">✓ V10"team-hero">
...
async function loadAll(){
  ...
  await Promise.allSettled([
    safeRunV10(fetchStaticESPN,18000),
    safeRunV10(fetchWCGames,3500),
    safeRunV10(fetchWCGroups,3500),
    safeRunV10(fetchWCScorers,3500),
    safeRunV10(fetchOFB,3500),
    safeRunV10(fetchFD,3500)
  ]);

  resolveKnockoutV23();
  if(typeof invalidateStatsCacheV10==="function")invalidateStatsCacheV10();
  render();
  if(typeof prewarmStatsCacheV10==="function")prewarmStatsCacheV10();
...
// ============================
// V10 - CAMADA ESTATÍSTICA NORMALIZADA
// ============================

function normMinuteV10(v){
...
function cardEventKeyV10(m,c){
  const eventId=c.id||c.eventId||c.uid||c.guid||"";
  if(eventId)return `${m.id}|id:${eventId}`;
  return [
    m.id,
    normPlayerV10(c.name||c.player||c.athlete||c.displayName||""),
    canon(c.team||""),
    cardType(c),
    normMinuteV10(c.minute||c.time||c.clock||"?")??"?"
  ].join("|");
}

function normalizeCardEventsV10(m,side){
  const team=side==="home"?m.h:m.a;
  ...
  raw.forEach(c=>{
    ...
    const type=cardType(c);
    const key=cardEventKeyV10(m,{...c,team});
    if(!map.has(key)){
      map.set(key,{
        id:c.id||c.eventId||null,
        name,
        playerKey:normPlayerV10(name),
        team,
        minute:normMinuteV10(c.minute||c.time||c.clock),
        card:type,
...
function teamOfficialCardTotalsV10(m,side){
...
function teamCardsForMatchV10(m,side){
  const events=normalizeCardEventsV10(m,side);
  const official=teamOfficialCardTotalsV10(m,side);
  ...
  // Regra V10: se houver total oficial, ele é soberano.
...
function tournamentTeamCardsV10(team){
  let yellow=0,red=0;
  const players=new Map();

  F.filter(m=>mSt(m)!=="upcoming"&&(nm(m.h,team)||nm(m.a,team))).forEach(m=>{
    const side=nm(m.h,team)?"home":"away";
    const pack=teamCardsForMatchV10(m,side);
    yellow+=pack.yellow;
    red+=pack.red;
...
function goalsByPlayerForTeamV10(team){
  const map=new Map();
  const seen=new Set();
    ...
    (allGoalsForMatchV9(m,ofb)||[])
      ...
      .forEach(g=>{
        ...
        const key=[
          ...
          normPlayerV10(name),
          canon(team),
          normMinuteV10(g.minute||g.time||g.clock)||"?"
        ].join("|");
        if(seen.has(key))return;
        seen.add(key);

        const pkey=normPlayerV10(name);
...
function aggregateTeamStatsV10(team){
  const out={
...
function v3Card(title,value,sub,details,icon="📊"){
  return `<div class="v3-card" onclick="toggleV10Card(this)">
    <div class="v3-head">
      <div>
...
function rowsV10(items){
  return items.map(x=>`<div class="v3-row"><span>${x[0]}</span><strong>${x[1]}</strong></div>`).join("");
}

function renderBrasilV10(){
  const s=aggregateTeamStatsV10("Brazil");
  const cards=tournamentTeamCardsV10("Brazil");
  const scorers=goalsByPlayerForTeamV10("Brazil");
  ...
  const scorersRows=scorers.length
    ? rowsV10(scorers.map(x=>[x.name,`${x.goals} gol${x.goals===1?"":"s"}`]))
    : '<div class="v3-muted">Sem gols identificados</div>';

  const discRows=discipline.length
    ? rowsV10(discipline.map(x=>[x.name,`${x.yellow} 🟨 · ${x.red} 🟥`]))
  ...
  return `
    ...
    <div class="v3-grid">
      ${v3Card("Jogos",s.games,`${s.wins}V · ${s.draws}E · ${s.losses}D`,
        rowsV10([["Aproveitamento",`${s.performance.toFixed(0)}%`],["Pontos",s.points],["Saldo",s.gd>=0?`+${s.gd}`:s.gd]]),"🏟️")}
      ...
      ${v3Card("Defesa",s.ga,`${s.cleanSheets} jogo${s.cleanSheets===1?"":"s"} sem sofrer`,
        rowsV10([["Gols sofridos",s.ga],["Saldo",s.gd],["Clean sheets",s.cleanSheets]]),"🛡️")}
      ${v3Card("Chutes",s.shots,s.shotAccuracy!==null?`${s.shotAccuracy.toFixed(1)}% no alvo`:"dados parciais",
        rowsV10([["Chutes",s.shots],["No alvo",s.shotsOnGoal],["Conversão",s.conversion!==null?`${s.conversion.toFixed(1)}%`:"—"]]),"🎯")}
      ${v3Card("Posse média",s.possessionAvg!==null?`${s.possessionAvg.toFixed(1)}%`:"—","média no torneio",
        rowsV10([["Posse média",s.possessionAvg!==null?`${s.possessionAvg.toFixed(1)}%`:"—"],["Precisão de passes",s.passAccuracyAvg!==null?`${s.passAccuracyAvg.toFixed(1)}%`:"—"],["Passes",s.passes||"—"]]),"🧠")}
      ${v3Card("Escanteios",s.corners,"total no torneio",
        rowsV10([["Escanteios",s.corners],["Faltas",s.fouls],["Impedimentos",s.offsides]]),"🚩")}
      ${v3Card("Disciplina",cards.yellow,`${cards.red} expulsão${cards.red===1?"":"ões"}`,
        rowsV10([["Amarelos",cards.yellow],["Vermelhos",cards.red],["Média/jogo",s.games?`${(cards.total/s.games).toFixed(2)}`:"—"]]),"⚖️")}
    </div>
...
function renderStatsV10(){
  ...
  const teamRows=teams.map(team=>{
    const st=aggregateTeamStatsV10(team);
    const cd=tournamentTeamCardsV10(team);
    return {team,...st,yellow:cd.yellow,red:cd.red,totalCards:cd.total};
  });

  const scorersMap=new Map();
  teams.forEach(team=>{
    goalsByPlayerForTeamV10(team).forEach(p=>{
      const k=normPlayerV10(p.name)+"|"+canon(team);
  ...
  return `
    ...
    <div class="v3-grid">
      ...
      ${v3Card("Maior posse",topPoss[0]?.possessionAvg!==null&&topPoss[0]?`${topPoss[0].possessionAvg.toFixed(1)}%`:"—",topPoss[0]?pt(topPoss[0].team):"dados parciais",
        rowsV10(topPoss.map(x=>[`${fl(x.team)} ${pt(x.team)}`,`${x.possessionAvg.toFixed(1)}%`])),"🧠")}
      ${v3Card("Mais chutes",Math.max(...teamRows.map(x=>x.shots),0),
        ...
        rowsV10([...teamRows].sort((a,b)=>b.shots-a.shots).slice(0,10).map(x=>[`${fl(x.team)} ${pt(x.team)}`,x.shots])),"🎯")}
      ${v3Card("Mais faltas",Math.max(...teamRows.map(x=>x.fouls),0),
        ...
        rowsV10([...teamRows].sort((a,b)=>b.fouls-a.fouls).slice(0,10).map(x=>[`${fl(x.team)} ${pt(x.team)}`,x.fouls])),"⚖️")}
    </div>
...
renderBrasil = renderBrasilV10;
renderStats = renderStatsV10;


// ============================
// V10 - ESTATÍSTICAS RÁPIDAS E CORRIGIDAS
// ============================

let STATS_CACHE_V10={key:"",model:null,building:false};

function invalidateStatsCacheV10(){
  STATS_CACHE_V10={key:"",model:null,building:false};
}

function statsFingerprintV10(){
...
function numberV10(v){
  if(v===null||v===undefined||v==="")return null;
  const n=Number(String(v).replace("%","").replace(",","."));
  return Number.isFinite(n)?n:null;
}

function statSideV10(stats,key,side){
  const map={
    shotsOnGoal:"shotsOnTarget",
    passes:"totalPasses"
  };
  const sourceKey=map[key]||key;
  return numberV10(stats?.[sourceKey]?.[side]);
}

function simpleCardEventsV10(m,side){
  const team=side==="home"?m.h:m.a;
  const raw=[];
  const ofb=ofbMatch(m.h,m.a);
  ...
  raw.forEach(c=>{
    ...
    const playerKey=normPlayerV10(name);
    const type=cardType(c);
    const minute=normMinuteV10(c.minute||c.time||c.clock);
...
function buildStatsModelV10(){
  ...
  const byTeam=new Map(teams.map(t=>[t,emptyTeamV10(t)]));
  ...
  const getTeam=name=>{
    ...
    if(!exact&&name&&!isPlaceholderTeamV23(name)){
      exact=name;
      teams.push(name);
      byTeam.set(name,emptyTeamV10(name));
    }
  ...
  F.filter(m=>mSt(m)==="finished"&&!isPlaceholderTeamV23(m.h)&&!isPlaceholderTeamV23(m.a))
   .forEach(m=>{
    ...
    [["home",home],["away",away]].forEach(([side,t])=>{
      ...
      const values={
        shots:statSideV10(stats,"shots",side),
        shotsOnGoal:statSideV10(stats,"shotsOnGoal",side),
        corners:statSideV10(stats,"corners",side),
        fouls:statSideV10(stats,"fouls",side),
        offsides:statSideV10(stats,"offsides",side),
        passes:statSideV10(stats,"passes",side),
        possession:statSideV10(stats,"possession",side),
        passAccuracy:statSideV10(stats,"passAccuracy",side)
      };
      ...
      const cards=cardPackV10(m,side);
    ...
    (allGoalsForMatchV9(m,ofb)||[])
      ...
      .forEach(g=>{
        ...
        const playerKey=normPlayerV10(name)+"|"+canon(team);
...
function getStatsModelV10(){
  const key=statsFingerprintV10();
  if(STATS_CACHE_V10.model&&STATS_CACHE_V10.key===key){
    return STATS_CACHE_V10.model;
  }

  const model=buildStatsModelV10();
  STATS_CACHE_V10={key,model,building:false};
  return model;
}

function prewarmStatsCacheV10(){
  setTimeout(()=>{
    try{getStatsModelV10()}catch(e){console.warn("V10 stats cache",e)}
  },80);
}

function teamModelV10(team){
  const model=getStatsModelV10();
  return model.teams.find(x=>nm(x.team,team))||emptyTeamV10(team);
}

function valueOrDashV10(v,suffix=""){
  return v===null||v===undefined||Number.isNaN(v)?"—":`${v}${suffix}`;
}

function renderBrasilFastV10(){
  const s=teamModelV10("Brazil");
  const model=getStatsModelV10();
  ...
  return `
    ...
    <div class="v3-grid">
      ...
      ${v3Card("Defesa",s.ga,`${s.cleanSheets} jogo${s.cleanSheets===1?"":"s"} sem sofrer`,
        rowsV10([["Gols sofridos",s.ga],["Saldo",s.gd],["Jogos sem sofrer",s.cleanSheets]]),"🛡️")}

      ${v3Card("Chutes",s.statsGames?valueOrDashV10(s.shots):"—",
        s.statsGames&&s.shotAccuracy!==null?`${s.shotAccuracy.toFixed(1)}% no alvo`:"dado ainda não disponível",
        rowsV10([["Chutes",s.statsGames?s.shots:"—"],["No alvo",s.statsGames?s.shotsOnGoal:"—"],["Conversão",s.conversion!==null?`${s.conversion.toFixed(1)}%`:"—"]]),"🎯")}
      ...
      ${v3Card("Posse média",s.possessionAvg!==null?`${s.possessionAvg.toFixed(1)}%`:"—",
        ...
        rowsV10([["Posse média",s.possessionAvg!==null?`${s.possessionAvg.toFixed(1)}%`:"—"],["Precisão de passes",s.passAccuracyAvg!==null?`${s.passAccuracyAvg.toFixed(1)}%`:"—"],["Passes",s.statsGames?s.passes:"—"]]),"🧠")}

      ${v3Card("Escanteios",s.statsGames?s.corners:"—",
        s.statsGames?"total no torneio":"dado ainda não disponível",
        rowsV10([["Escanteios",s.statsGames?s.corners:"—"],["Faltas",s.statsGames?s.fouls:"—"],["Impedimentos",s.statsGames?s.offsides:"—"]]),"🚩")}

      ${v3Card("Disciplina",s.yellow,`${s.red} expulsão${s.red===1?"":"ões"}`,
        rowsV10([["Amarelos",s.yellow],["Vermelhos",s.red],["Média por jogo",s.games?(s.totalCards/s.games).toFixed(2):"—"]]),"⚖️")}
    </div>
  ...
  return `
    ...
    <div class="v3-grid">
      ...
      ${v3Card("Artilheiros",model.scorers[0]?.goals||"—",
        ...
        rowsV10(model.scorers.slice(0,20).map(x=>[`${x.name} · ${pt(x.team)}`,x.goals])),"🥇")}

      ${v3Card("Cartões",totalYellow+totalRed,
        `${totalYellow} amarelos · ${totalRed} vermelhos`,
        topCards.length?rowsV10(topCards.map(x=>[`${fl(x.team)} ${pt(x.team)}`,`${x.yellow} 🟨 · ${x.red} 🟥`])):'<div class="v3-muted">Dados ainda não disponíveis</div>',"🟨")}
      ...
      ${v3Card("Maior posse",topPoss[0]?`${topPoss[0].possessionAvg.toFixed(1)}%`:"—",
        ...
        topPoss.length?rowsV10(topPoss.map(x=>[`${fl(x.team)} ${pt(x.team)}`,`${x.possessionAvg.toFixed(1)}%`])):'<div class="v3-muted">A fonte ainda não entregou posse consolidada</div>',"🧠")}

      ${v3Card("Mais chutes",topShots[0]?.shots??"—",
        topShots[0]?pt(topShots[0].team):"dado ainda não disponível",
        topShots.length?rowsV10(topShots.map(x=>[`${fl(x.team)} ${pt(x.team)}`,x.shots])):'<div class="v3-muted">A fonte ainda não entregou chutes consolidados</div>',"🎯")}

      ${v3Card("Mais faltas",topFouls[0]?.fouls??"—",
        topFouls[0]?pt(topFouls[0].team):"dado ainda não disponível",
        topFouls.length?rowsV10(topFouls.map(x=>[`${fl(x.team)} ${pt(x.team)}`,x.fouls])):'<div class="v3-muted">A fonte ainda não entregou faltas consolidadas</div>',"⚖️")}
    </div>
...
renderBrasil=renderBrasilFastV10;
renderStats=renderStatsFastV10;


// ============================
// V10 - MELHORES TERCEIROS NO MATA-MATA
// ============================

function groupThirdsV10(){
  const out=[];
...
function bestThirdTeamsV10(){
  return groupThirdsV10().slice(0,8);
}

function isBestThirdSlotV10(slot){
  return /3º colocado|melhor\s*3/i.test(String(slot||""));
}

function applyBestThirdsV10(){
  const thirds=bestThirdTeamsV10();
  ...
  F.filter(m=>/^e\d+$/i.test(m.id)).forEach(m=>{
    [["h","slotH"],["a","slotA"]].forEach(([side,slotKey])=>{
      ...
      if(!isBestThirdSlotV10(slot))return;
      if(!isPlaceholderTeamV23(m[side]) && !isBestThirdSlotV10(m[side]))return;
...
const resolveKnockoutBaseV10 = resolveKnockoutV23;
resolveKnockoutV23 = function(){
  resolveKnockoutBaseV10();
  applyBestThirdsV10();
  applyOfficialParticipantsV23();
...
// ============================
// V10 - PLACAR COM PRORROGAÇÃO E PÊNALTIS
// ============================
...
function readPenaltyScoreV10(c){
  if(!c)return null;
...
function penaltyInfoV10(m){
  ...
  const hp=readPenaltyScoreV10(home);
  const ap=readPenaltyScoreV10(away);
...
function statusIsPenaltyV10(m,data){
  const txt=[
    data?.min,
    espnStatusFor(m)?.min,
...
const winnerLoserBaseV10 = winnerLoserV23;
winnerLoserV23 = function(m){
  const base=winnerLoserBaseV10(m);
  if(base)return base;

  const pk=penaltyInfoV10(m);
  if(pk)return {winner:pk.winner,loser:pk.loser};

  return null;
};

const mkCardBaseV10 = mkCard;
mkCard = function(m){
  const data=mData(m);
  ...
  let pill="";
  if(st==="live"){
    const pen=statusIsPenaltyV10(m,data);
...
// ============================
// V10 - ESTABILIDADE
// Correção simples: dedupe de gols + trava de placar futuro
// Sem sobrescrever mData, mkCard ou openModal.
// ============================

function normalizeGoalMinuteV10(v){
  const s=String(v??"").trim();
  if(!s)return "?";
  const plus=s.match(/(\d+)\s*\+\s*(\d+)/);
...
function normalizeGoalNameV10(name){
  return canon(
    String(name||"")
      .replace(/\([^)]*\)/g," ")
      .replace(/\s+/g," ")
...
function goalDedupeV10(goals,m){
  const seen=new Set();
  return (goals||[]).filter(g=>{
    ...
    const key=[
      m.id,
      normalizeGoalNameV10(name),
      canon(team),
      normalizeGoalMinuteV10(g.minute||g.time||g.clock||g.displayClock)
    ].join("|");
    if(seen.has(key))return false;
    seen.add(key);
    return true;
...
const allGoalsForMatchBaseV10 = allGoalsForMatchV9;
allGoalsForMatchV9 = function(m,ofb){
  return goalDedupeV10(allGoalsForMatchBaseV10(m,ofb),m);
};

goalsSummaryV9 = function(m){
  const ofb=ofbMatch(m.h,m.a);
  const goals=goalDedupeV10(allGoalsForMatchBaseV10(m,ofb),m);
  if(!goals.length)return "";
  const home=[],away=[];
  goals.forEach(g=>{
    const txt=`${fl(g.team)} ${g.name}${g.minute?` ${g.minute}'`:""}`;
    if(sameTeamV16(g.team,m.h))home.push(txt);
    else if(sameTeamV16(g.team,m.a))away.push(txt);
    else home.push(txt);
  });
  return `<div class="goal-line">${[...home,...away].join(" | ")}</div>`;
};

// Trava leve contra placar indevido em jogos futuros.
// Não sobrescreve mData. Apenas impede que o casamento por proximidade use evento errado.
const espnEventByFixtureBaseV10 = espnEventByFixtureV23;
espnEventByFixtureV23 = function(m){
  ...
  const hasRealTeams=!isPlaceholderTeamV23(m.h)&&!isPlaceholderTeamV23(m.a);

  if(hasRealTeams){
    const exact=sameDay.find(ev=>{
      const t=espnTeamsForEventV23(ev);
      return (sameTeamV16(t.home,m.h)&&sameTeamV16(t.away,m.a)) ||
    ...
    // Jogo com times reais, mas sem casamento exato: não aplica evento por aproximação.
    return null;
  }

  // Placeholder futuro: não aplica placar por aproximação.
  if(mSt(m)==="upcoming")return null;

  return espnEventByFixtureBaseV10(m);
};

console.log('Copa 2026 V10 carregado');
