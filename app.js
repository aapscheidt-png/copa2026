...
const ESPN_CACHE_KEY_V22="copa2026_data_v3_chave";
...
// ============================
// V3 - PLACAR FINAL PELO EVENTO OFICIAL
// Para mata-mata, data + horário oficial também alimentam o placar.
// Isso corrige jogos como México x Bósnia-Herz. quando o adversário
// foi corrigido pela agenda, mas o placar não entrou por diferença de nome.
// ============================

function eventDataByOfficialDateTimeV3(m){
  if(typeof officialEventByDateTimeV3!=="function")return null;
  if(m.ph==="grupos")return null;
  ...
  const isDone=!!(typ.completed || state==="post");

  // Não aplica placar em jogo futuro.
  ...
  const st=isDone?"finished":"live";
  const clock=comp?.status?.displayClock || ev?.status?.displayClock || typ.shortDetail || typ.detail || "";

  let pct=st==="finished"?100:8;
  const rawClock=Number(comp?.status?.clock||ev?.status?.clock||0);
  if(st==="live"&&Number.isFinite(rawClock)&&rawClock>0){
    pct=Math.min(100,Math.max(4,Math.round((rawClock/5400)*100)));
  }
  ...
  return {
    ...
    min:clock,
    pct,
    source:"ESPN oficial horário"
  };
}

const mDataBeforeOfficialScoreV3 = mData;
mData = function(m){
  const official=eventDataByOfficialDateTimeV3(m);
  if(official)return official;

  const base=mDataBeforeOfficialScoreV3(m);

  // Se alguma fonte marcar jogo futuro como FIM sem placar,
  // evita mostrar FIM + horário sem resultado.
  if(base && base.st==="finished" && !base.hasScore){
    const kick=matchKick(m).getTime();
    if(Date.now() < kick + 150*60*1000){
      return {st:"upcoming",hasScore:false,hs:null,as:null,min:"",pct:0,source:"proteção futuro"};
    }
  }
