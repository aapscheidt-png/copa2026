...
const ESPN_CACHE_KEY_V22="copa2026_data_v9";
...
function manualLiveV9(m){
  const x = LIVE_MATCHES[matchKey(m.h,m.a)];
  if(!x) return null;
  const clk = liveClockV9(x.startISO);
  return {
    hs:x.hs,
    as:x.as,
    hasScore:true,
...
// V9 - Base disciplinar local do torneio.
...
function cardPlayerNameV9(c){return c.name||c.player||c.athlete||c.displayName||c.text||"Jogador não informado";}
function cardMinuteV9(c){return c.minute||c.time||c.clock||"?";}
...
function renderStats(){
  ...
  const teams={};
  allTournamentTeamsV9().forEach(t=>teams[t]={nm:t,j:0,v:0,e:0,d:0,gp:0,gc:0,sg:0,pts:0,cs:0,yc:0,rc:0,totalCards:0,agg:false});
  ...
  F.filter(m=>mSt(m)!=="upcoming").forEach(m=>{
    ["home","away"].forEach(side=>{
      ...
      cards.forEach(c=>{
        const type=cardType(c), name=cardPlayerNameV9(c), minute=cardMinuteV9(c);
  ...
  return`<div class="stats-version">✓ V9"kpi-grid">
...
<table class="info-tbl">
  ...
  <tr><td>Versão</td><td style="color:var(--gold)">V9</td></tr>
</table></div>`;
}
...
function renderBrasil(){
  ...
  return `<div class="pro-badge">✓ V9"team-hero">
...
async function loadAll(){
  ...
  await Promise.allSettled([
    safeRunV9(fetchStaticESPN,18000),
    safeRunV9(fetchWCGames,3500),
    safeRunV9(fetchWCGroups,3500),
    safeRunV9(fetchWCScorers,3500),
    safeRunV9(fetchOFB,3500),
    safeRunV9(fetchFD,3500)
  ]);

  resolveKnockoutV23();
  if(typeof invalidateStatsCacheV9==="function")invalidateStatsCacheV9();
  render();
  if(typeof prewarmStatsCacheV9==="function")prewarmStatsCacheV9();
...
// ============================
// V9 - CAMADA ESTATÍSTICA NORMALIZADA
// ============================

function normMinuteV9(v){
...
function cardEventKeyV9(m,c){
  const eventId=c.id||c.eventId||c.uid||c.guid||"";
  if(eventId)return `${m.id}|id:${eventId}`;
  return [
    m.id,
    normPlayerV9(c.name||c.player||c.athlete||c.displayName||""),
    canon(c.team||""),
    cardType(c),
    normMinuteV9(c.minute||c.time||c.clock||"?")??"?"
  ].join("|");
}

function normalizeCardEventsV9(m,side){
  const team=side==="home"?m.h:m.a;
  ...
  raw.forEach(c=>{
    ...
    const type=cardType(c);
    const key=cardEventKeyV9(m,{...c,team});
    if(!map.has(key)){
      map.set(key,{
        id:c.id||c.eventId||null,
        name,
        playerKey:normPlayerV9(name),
        team,
        minute:normMinuteV9(c.minute||c.time||c.clock),
        card:type,
...
function teamOfficialCardTotalsV9(m,side){
...
function teamCardsForMatchV9(m,side){
  const events=normalizeCardEventsV9(m,side);
  const official=teamOfficialCardTotalsV9(m,side);
  ...
  // Regra V9: se houver total oficial, ele é soberano.
...
function tournamentTeamCardsV9(team){
  let yellow=0,red=0;
  const players=new Map();

  F.filter(m=>mSt(m)!=="upcoming"&&(nm(m.h,team)||nm(m.a,team))).forEach(m=>{
    const side=nm(m.h,team)?"home":"away";
    const pack=teamCardsForMatchV9(m,side);
    yellow+=pack.yellow;
    red+=pack.red;
...
function goalsByPlayerForTeamV9(team){
  const map=new Map();
  const seen=new Set();
    ...
    (allGoalsForMatchV9(m,ofb)||[])
      ...
      .forEach(g=>{
        ...
        const key=[
          ...
          normPlayerV9(name),
          canon(team),
          normMinuteV9(g.minute||g.time||g.clock)||"?"
        ].join("|");
        if(seen.has(key))return;
        seen.add(key);

        const pkey=normPlayerV9(name);
...
function aggregateTeamStatsV9(team){
  const out={
...
function v3Card(title,value,sub,details,icon="📊"){
  return `<div class="v3-card" onclick="toggleV9Card(this)">
    <div class="v3-head">
      <div>
...
function rowsV9(items){
  return items.map(x=>`<div class="v3-row"><span>${x[0]}</span><strong>${x[1]}</strong></div>`).join("");
}

function renderBrasilV9(){
  const s=aggregateTeamStatsV9("Brazil");
  const cards=tournamentTeamCardsV9("Brazil");
  const scorers=goalsByPlayerForTeamV9("Brazil");
  ...
  const scorersRows=scorers.length
    ? rowsV9(scorers.map(x=>[x.name,`${x.goals} gol${x.goals===1?"":"s"}`]))
    : '<div class="v3-muted">Sem gols identificados</div>';

  const discRows=discipline.length
    ? rowsV9(discipline.map(x=>[x.name,`${x.yellow} 🟨 · ${x.red} 🟥`]))
  ...
  return `
    ...
    <div class="v3-grid">
      ${v3Card("Jogos",s.games,`${s.wins}V · ${s.draws}E · ${s.losses}D`,
        rowsV9([["Aproveitamento",`${s.performance.toFixed(0)}%`],["Pontos",s.points],["Saldo",s.gd>=0?`+${s.gd}`:s.gd]]),"🏟️")}
      ...
      ${v3Card("Defesa",s.ga,`${s.cleanSheets} jogo${s.cleanSheets===1?"":"s"} sem sofrer`,
        rowsV9([["Gols sofridos",s.ga],["Saldo",s.gd],["Clean sheets",s.cleanSheets]]),"🛡️")}
      ${v3Card("Chutes",s.shots,s.shotAccuracy!==null?`${s.shotAccuracy.toFixed(1)}% no alvo`:"dados parciais",
        rowsV9([["Chutes",s.shots],["No alvo",s.shotsOnGoal],["Conversão",s.conversion!==null?`${s.conversion.toFixed(1)}%`:"—"]]),"🎯")}
      ${v3Card("Posse média",s.possessionAvg!==null?`${s.possessionAvg.toFixed(1)}%`:"—","média no torneio",
        rowsV9([["Posse média",s.possessionAvg!==null?`${s.possessionAvg.toFixed(1)}%`:"—"],["Precisão de passes",s.passAccuracyAvg!==null?`${s.passAccuracyAvg.toFixed(1)}%`:"—"],["Passes",s.passes||"—"]]),"🧠")}
      ${v3Card("Escanteios",s.corners,"total no torneio",
        rowsV9([["Escanteios",s.corners],["Faltas",s.fouls],["Impedimentos",s.offsides]]),"🚩")}
      ${v3Card("Disciplina",cards.yellow,`${cards.red} expulsão${cards.red===1?"":"ões"}`,
        rowsV9([["Amarelos",cards.yellow],["Vermelhos",cards.red],["Média/jogo",s.games?`${(cards.total/s.games).toFixed(2)}`:"—"]]),"⚖️")}
    </div>
...
function renderStatsV9(){
  ...
  const teamRows=teams.map(team=>{
    const st=aggregateTeamStatsV9(team);
    const cd=tournamentTeamCardsV9(team);
    return {team,...st,yellow:cd.yellow,red:cd.red,totalCards:cd.total};
  });

  const scorersMap=new Map();
  teams.forEach(team=>{
    goalsByPlayerForTeamV9(team).forEach(p=>{
      const k=normPlayerV9(p.name)+"|"+canon(team);
  ...
  return `
    ...
    <div class="v3-grid">
      ...
      ${v3Card("Maior posse",topPoss[0]?.possessionAvg!==null&&topPoss[0]?`${topPoss[0].possessionAvg.toFixed(1)}%`:"—",topPoss[0]?pt(topPoss[0].team):"dados parciais",
        rowsV9(topPoss.map(x=>[`${fl(x.team)} ${pt(x.team)}`,`${x.possessionAvg.toFixed(1)}%`])),"🧠")}
      ${v3Card("Mais chutes",Math.max(...teamRows.map(x=>x.shots),0),
        ...
        rowsV9([...teamRows].sort((a,b)=>b.shots-a.shots).slice(0,10).map(x=>[`${fl(x.team)} ${pt(x.team)}`,x.shots])),"🎯")}
      ${v3Card("Mais faltas",Math.max(...teamRows.map(x=>x.fouls),0),
        ...
        rowsV9([...teamRows].sort((a,b)=>b.fouls-a.fouls).slice(0,10).map(x=>[`${fl(x.team)} ${pt(x.team)}`,x.fouls])),"⚖️")}
    </div>
...
renderBrasil = renderBrasilV9;
renderStats = renderStatsV9;


// ============================
// V9 - ESTATÍSTICAS RÁPIDAS E CORRIGIDAS
// ============================

let STATS_CACHE_V9={key:"",model:null,building:false};

function invalidateStatsCacheV9(){
  STATS_CACHE_V9={key:"",model:null,building:false};
}

function statsFingerprintV9(){
...
function statSideV9(stats,key,side){
  const map={
    shotsOnGoal:"shotsOnTarget",
    passes:"totalPasses"
  };
  const sourceKey=map[key]||key;
  return numberV9(stats?.[sourceKey]?.[side]);
}

function simpleCardEventsV9(m,side){
  const team=side==="home"?m.h:m.a;
  const raw=[];
  const ofb=ofbMatch(m.h,m.a);
  ...
  raw.forEach(c=>{
    ...
    const playerKey=normPlayerV9(name);
    const type=cardType(c);
    const minute=normMinuteV9(c.minute||c.time||c.clock);
...
function buildStatsModelV9(){
  ...
  const byTeam=new Map(teams.map(t=>[t,emptyTeamV9(t)]));
  ...
  const getTeam=name=>{
    ...
    if(!exact&&name&&!isPlaceholderTeamV23(name)){
      exact=name;
      teams.push(name);
      byTeam.set(name,emptyTeamV9(name));
    }
  ...
  F.filter(m=>mSt(m)==="finished"&&!isPlaceholderTeamV23(m.h)&&!isPlaceholderTeamV23(m.a))
   .forEach(m=>{
    ...
    [["home",home],["away",away]].forEach(([side,t])=>{
      ...
      const values={
        shots:statSideV9(stats,"shots",side),
        shotsOnGoal:statSideV9(stats,"shotsOnGoal",side),
        corners:statSideV9(stats,"corners",side),
        fouls:statSideV9(stats,"fouls",side),
        offsides:statSideV9(stats,"offsides",side),
        passes:statSideV9(stats,"passes",side),
        possession:statSideV9(stats,"possession",side),
        passAccuracy:statSideV9(stats,"passAccuracy",side)
      };
    ...
    (allGoalsForMatchV9(m,ofb)||[])
      ...
      .forEach(g=>{
        ...
        const playerKey=normPlayerV9(name)+"|"+canon(team);
...
function getStatsModelV9(){
  const key=statsFingerprintV9();
  if(STATS_CACHE_V9.model&&STATS_CACHE_V9.key===key){
    return STATS_CACHE_V9.model;
  }

  const model=buildStatsModelV9();
  STATS_CACHE_V9={key,model,building:false};
  return model;
}

function prewarmStatsCacheV9(){
  setTimeout(()=>{
    try{getStatsModelV9()}catch(e){console.warn("V9 stats cache",e)}
  },80);
}

function teamModelV9(team){
  const model=getStatsModelV9();
  return model.teams.find(x=>nm(x.team,team))||emptyTeamV9(team);
}

function valueOrDashV9(v,suffix=""){
  return v===null||v===undefined||Number.isNaN(v)?"—":`${v}${suffix}`;
}

function renderBrasilFastV9(){
  const s=teamModelV9("Brazil");
  const model=getStatsModelV9();
  ...
  return `
    ...
    <div class="v3-grid">
      ...
      ${v3Card("Defesa",s.ga,`${s.cleanSheets} jogo${s.cleanSheets===1?"":"s"} sem sofrer`,
        rowsV9([["Gols sofridos",s.ga],["Saldo",s.gd],["Jogos sem sofrer",s.cleanSheets]]),"🛡️")}

      ${v3Card("Chutes",s.statsGames?valueOrDashV9(s.shots):"—",
        s.statsGames&&s.shotAccuracy!==null?`${s.shotAccuracy.toFixed(1)}% no alvo`:"dado ainda não disponível",
        rowsV9([["Chutes",s.statsGames?s.shots:"—"],["No alvo",s.statsGames?s.shotsOnGoal:"—"],["Conversão",s.conversion!==null?`${s.conversion.toFixed(1)}%`:"—"]]),"🎯")}
      ...
      ${v3Card("Posse média",s.possessionAvg!==null?`${s.possessionAvg.toFixed(1)}%`:"—",
        ...
        rowsV9([["Posse média",s.possessionAvg!==null?`${s.possessionAvg.toFixed(1)}%`:"—"],["Precisão de passes",s.passAccuracyAvg!==null?`${s.passAccuracyAvg.toFixed(1)}%`:"—"],["Passes",s.statsGames?s.passes:"—"]]),"🧠")}

      ${v3Card("Escanteios",s.statsGames?s.corners:"—",
        s.statsGames?"total no torneio":"dado ainda não disponível",
        rowsV9([["Escanteios",s.statsGames?s.corners:"—"],["Faltas",s.statsGames?s.fouls:"—"],["Impedimentos",s.statsGames?s.offsides:"—"]]),"🚩")}

      ${v3Card("Disciplina",s.yellow,`${s.red} expulsão${s.red===1?"":"ões"}`,
        rowsV9([["Amarelos",s.yellow],["Vermelhos",s.red],["Média por jogo",s.games?(s.totalCards/s.games).toFixed(2):"—"]]),"⚖️")}
    </div>
  ...
  return `
    ...
    <div class="v3-grid">
      ...
      ${v3Card("Artilheiros",model.scorers[0]?.goals||"—",
        ...
        rowsV9(model.scorers.slice(0,20).map(x=>[`${x.name} · ${pt(x.team)}`,x.goals])),"🥇")}

      ${v3Card("Cartões",totalYellow+totalRed,
        `${totalYellow} amarelos · ${totalRed} vermelhos`,
        topCards.length?rowsV9(topCards.map(x=>[`${fl(x.team)} ${pt(x.team)}`,`${x.yellow} 🟨 · ${x.red} 🟥`])):'<div class="v3-muted">Dados ainda não disponíveis</div>',"🟨")}
      ...
      ${v3Card("Maior posse",topPoss[0]?`${topPoss[0].possessionAvg.toFixed(1)}%`:"—",
        ...
        topPoss.length?rowsV9(topPoss.map(x=>[`${fl(x.team)} ${pt(x.team)}`,`${x.possessionAvg.toFixed(1)}%`])):'<div class="v3-muted">A fonte ainda não entregou posse consolidada</div>',"🧠")}

      ${v3Card("Mais chutes",topShots[0]?.shots??"—",
        topShots[0]?pt(topShots[0].team):"dado ainda não disponível",
        topShots.length?rowsV9(topShots.map(x=>[`${fl(x.team)} ${pt(x.team)}`,x.shots])):'<div class="v3-muted">A fonte ainda não entregou chutes consolidados</div>',"🎯")}

      ${v3Card("Mais faltas",topFouls[0]?.fouls??"—",
        topFouls[0]?pt(topFouls[0].team):"dado ainda não disponível",
        topFouls.length?rowsV9(topFouls.map(x=>[`${fl(x.team)} ${pt(x.team)}`,x.fouls])):'<div class="v3-muted">A fonte ainda não entregou faltas consolidadas</div>',"⚖️")}
    </div>
...
renderBrasil=renderBrasilFastV9;
renderStats=renderStatsFastV9;


// ============================
// V9 - MELHORES TERCEIROS NO MATA-MATA
// ============================

function groupThirdsV9(){
  const out=[];
...
function bestThirdTeamsV9(){
  return groupThirdsV9().slice(0,8);
}

function isBestThirdSlotV9(slot){
  return /3º colocado|melhor\s*3/i.test(String(slot||""));
}

function applyBestThirdsV9(){
  const thirds=bestThirdTeamsV9();
  ...
  F.filter(m=>/^e\d+$/i.test(m.id)).forEach(m=>{
    [["h","slotH"],["a","slotA"]].forEach(([side,slotKey])=>{
      ...
      if(!isBestThirdSlotV9(slot))return;
      if(!isPlaceholderTeamV23(m[side]) && !isBestThirdSlotV9(m[side]))return;
...
const resolveKnockoutBaseV9 = resolveKnockoutV23;
resolveKnockoutV23 = function(){
  resolveKnockoutBaseV9();
  applyBestThirdsV9();
  applyOfficialParticipantsV23();
...
// ============================
// V9 - PLACAR COM PRORROGAÇÃO E PÊNALTIS
// ============================
...
function penaltyInfoV9(m){
  ...
  const hp=readPenaltyScoreV9(home);
  const ap=readPenaltyScoreV9(away);
...
function statusIsPenaltyV9(m,data){
  const txt=[
    data?.min,
    espnStatusFor(m)?.min,
...
const winnerLoserBaseV9 = winnerLoserV23;
winnerLoserV23 = function(m){
  const base=winnerLoserBaseV9(m);
  if(base)return base;

  const pk=penaltyInfoV9(m);
  if(pk)return {winner:pk.winner,loser:pk.loser};

  return null;
};

const mkCardBaseV9 = mkCard;
mkCard = function(m){
  const data=mData(m);
  ...
  let pill="";
  if(st==="live"){
    const pen=statusIsPenaltyV9(m,data);
...
// ============================
// V9 - CORREÇÃO DE DUPLICIDADE E CASAMENTO DE JOGOS
// ============================

function normalizeClockGoalV9(v){
  const s=String(v??"").trim();
  if(!s)return "?";
  const plus=s.match(/(\d+)\s*\+\s*(\d+)/);
...
function strictFixtureEventV9(m){
...
function safeScoreForMatchV9(m){
  const st=mSt(m);
  const ev=strictFixtureEventV9(m);

  if(st==="upcoming" && !ev){
    return {st:"upcoming",hasScore:false,hs:null,as:null,min:""};
  }

  return mDataBaseV9Safe(m);
}

function dedupeGoalsV9(goals,m){
  const seen=new Set();
  return (goals||[]).filter(g=>{
    ...
    const key=[
      m.id,
      playerKeyGoalV9(name),
      canon(team),
      normalizeClockGoalV9(g.minute||g.time||g.clock||g.displayClock)
    ].join("|");
...
const mDataBaseV9Safe = mData;

const allGoalsForMatchBaseV9Safe = allGoalsForMatchV9;
allGoalsForMatchV9 = function(m,ofb){
  return dedupeGoalsV9(allGoalsForMatchBaseV9Safe(m,ofb),m);
};

goalsSummaryV9 = function(m){
  const ofb=ofbMatch(m.h,m.a);
  const goals=dedupeGoalsV9(allGoalsForMatchBaseV9Safe(m,ofb),m);
  if(!goals.length)return "";
  const h=[],a=[];
  goals.forEach(g=>{
    const txt=`${fl(g.team)} ${g.name}${g.minute?` ${g.minute}'`:""}`;
    if(sameTeamV16(g.team,m.h))h.push(txt);
    else if(sameTeamV16(g.team,m.a))a.push(txt);
    else h.push(txt);
  });
  return `<div class="goal-line">${[...h,...a].join(" | ")}</div>`;
};

const espnEventByFixtureBaseV9Safe = espnEventByFixtureV23;
espnEventByFixtureV23 = function(m){
  const strict=strictFixtureEventV9(m);
  if(strict)return strict;

  if(isPlaceholderTeamV23(m.h)||isPlaceholderTeamV23(m.a)){
    const status=mSt(m);
    if(status!=="upcoming")return espnEventByFixtureBaseV9Safe(m);
  }
  return null;
};

const mkCardBaseV9Safe = mkCard;
mkCard = function(m){
  const originalMData=mData;
  try{
    mData=function(mm){
      if(mm&&mm.id===m.id)return safeScoreForMatchV9(mm);
      return originalMData(mm);
    };
    return mkCardBaseV9Safe(m);
  }finally{
    mData=originalMData;
  }
};

const openModalBaseV9Safe = openModal;
openModal = function(id){
  const originalMData=mData;
  try{
    mData=function(mm){
      if(mm&&mm.id===id)return safeScoreForMatchV9(mm);
      return originalMData(mm);
    };
    return openModalBaseV9Safe(id);
  }finally{
    mData=originalMData;
  }
};

console.log('Copa 2026 V9 carregado');
