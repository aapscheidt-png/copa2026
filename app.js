...
const ESPN_CACHE_KEY_V22="copa2026_data_v3_matamata_oficial";
...
function manualLiveV3(m){
  const x = LIVE_MATCHES[matchKey(m.h,m.a)];
  if(!x) return null;
  const clk = liveClockV3(x.startISO);
  return {
    hs:x.hs,
    as:x.as,
    hasScore:true,
...
function buildDetail(m,ofb){
  ...
  const goals=allGoalsForMatchV3(m,ofb);
  if(goals.length){
  ...
  const cards=dedupeEventsV3([...b1.map(b=>({...b,side:"home",team:m.h})),...b2.map(b=>({...b,side:"away",team:m.a}))])
...
// V3 - Base disciplinar local do torneio.
...
// V3 - Totais agregados por seleção quando a fonte informa o total, mas não todos os jogadores.
...
function teamDisciplineTotalV3(team){
...
function dedupeEventsV3(arr){
  const seen=new Set();
  return (arr||[]).filter(e=>{
    const k=eventKeyV3(e);
    if(seen.has(k))return false;
    seen.add(k);
    return true;
  });
}
function localGoalsForMatchV3(m){
  const k=matchKey(m.h,m.a);
  ...
  return dedupeEventsV3(ev.map(g=>({name:g.name||g.player,team:g.team,minute:g.minute,penalty:g.penalty,owngoal:g.owngoal,type:g.type||"goal"})));
}
function allGoalsForMatchV3(m,ofb){
  const goals=[];
  if(ofb){
  ...
  localGoalsForMatchV3(m).forEach(g=>goals.push({...g,side:nm(g.team,m.h)?"home":"away"}));
  return dedupeEventsV3(goals).sort((a,b)=>parseInt(a.minute||999)-parseInt(b.minute||999));
}
function goalsSummaryV3(m){
  const ofb=ofbMatch(m.h,m.a);
  const goals=allGoalsForMatchV3(m,ofb);
...
function cardPlayerNameV3(c){return c.name||c.player||c.athlete||c.displayName||c.text||"Jogador não informado";}
function cardMinuteV3(c){return c.minute||c.time||c.clock||"?";}
...
function renderStats(){
  ...
  const teams={};
  allTournamentTeamsV3().forEach(t=>teams[t]={nm:t,j:0,v:0,e:0,d:0,gp:0,gc:0,sg:0,pts:0,cs:0,yc:0,rc:0,totalCards:0,agg:false});
  ...
  F.filter(m=>mSt(m)!=="upcoming").forEach(m=>{
    ["home","away"].forEach(side=>{
      ...
      cards.forEach(c=>{
        const type=cardType(c), name=cardPlayerNameV3(c), minute=cardMinuteV3(c);
  ...
  Object.values(teams).forEach(t=>{
    ...
    const agg=teamDisciplineTotalV3(t.nm);
  ...
  // dentro de allGoalsForMatchV3(). Agora o ranking nasce de uma única lista
  ...
  return`<div class="stats-version">✓ V3"kpi-grid">
...
<table class="info-tbl">
  ...
  <tr><td>Versão</td><td style="color:var(--gold)">V3</td></tr>
</table></div>`;
}
...
function teamStatsV12(team){
  ...
  games.filter(m=>mSt(m)!=="upcoming").forEach(m=>{
    const ofb=ofbMatch(m.h,m.a);
    allGoalsForMatchV3(m,ofb).filter(g=>nm(g.team,team)).forEach(g=>{
...
function renderBrasil(){
  ...
  return `<div class="pro-badge">✓ V3"team-hero">
...
function playerBadgesV20(m,team,name){
  const ofb=ofbMatch(m.h,m.a);
  const goals=allGoalsForMatchV3(m,ofb).filter(g=>sameTeamV16(g.team,team)&&canon(g.name)===canon(name));
...
function renderSubsV20(m,ofb){
  ...
  const ded=dedupeEventsV3(arr.map(x=>({name:`${x.in}|${x.out}`,team:x.team,minute:x.minute,type:"sub",...x}))).sort((a,b)=>parseInt(a.minute||999)-parseInt(b.minute||999));
...
function buildDetail(m,ofb){
  ...
  const b1=cardsForMatch(m,"home"), b2=cardsForMatch(m,"away");
  const cards=dedupeEventsV3([...b1.map(b=>({...b,side:"home",team:m.h})),...b2.map(b=>({...b,side:"away",team:m.a}))]).sort((a,b)=>parseInt(a.minute||999)-parseInt(b.minute||999));
  if(cards.length){
    html+='<div class="modal-sec"><div class="modal-sec-title">🟨 Cartões</div>';
    cards.forEach(b=>{
      const isH=b.side==="home";
...
async function loadAll(){
  ...
  await Promise.allSettled([
    safeRunV3(fetchStaticESPN,18000),
    safeRunV3(fetchWCGames,3500),
    safeRunV3(fetchWCGroups,3500),
    safeRunV3(fetchWCScorers,3500),
    safeRunV3(fetchOFB,3500),
    safeRunV3(fetchFD,3500)
  ]);

  resolveKnockoutV23();
  if(typeof invalidateStatsCacheV3==="function")invalidateStatsCacheV3();
  render();
  if(typeof prewarmStatsCacheV3==="function")prewarmStatsCacheV3();
...
// ============================
// V3 - CAMADA ESTATÍSTICA NORMALIZADA
// ============================

function normMinuteV3(v){
...
function cardEventKeyV3(m,c){
  const eventId=c.id||c.eventId||c.uid||c.guid||"";
  if(eventId)return `${m.id}|id:${eventId}`;
  return [
    m.id,
    normPlayerV3(c.name||c.player||c.athlete||c.displayName||""),
    canon(c.team||""),
    cardType(c),
    normMinuteV3(c.minute||c.time||c.clock||"?")??"?"
  ].join("|");
}

function normalizeCardEventsV3(m,side){
  const team=side==="home"?m.h:m.a;
  ...
  raw.forEach(c=>{
    ...
    const type=cardType(c);
    const key=cardEventKeyV3(m,{...c,team});
    if(!map.has(key)){
      map.set(key,{
        id:c.id||c.eventId||null,
        name,
        playerKey:normPlayerV3(name),
        team,
        minute:normMinuteV3(c.minute||c.time||c.clock),
        card:type,
...
function teamOfficialCardTotalsV3(m,side){
...
function teamCardsForMatchV3(m,side){
  const events=normalizeCardEventsV3(m,side);
  const official=teamOfficialCardTotalsV3(m,side);
  ...
  // Regra V3: se houver total oficial, ele é soberano.
...
function tournamentTeamCardsV3(team){
  let yellow=0,red=0;
  const players=new Map();

  F.filter(m=>mSt(m)!=="upcoming"&&(nm(m.h,team)||nm(m.a,team))).forEach(m=>{
    const side=nm(m.h,team)?"home":"away";
    const pack=teamCardsForMatchV3(m,side);
    yellow+=pack.yellow;
    red+=pack.red;
...
function goalsByPlayerForTeamV3(team){
  const map=new Map();
  const seen=new Set();
    ...
    (allGoalsForMatchV3(m,ofb)||[])
      ...
      .forEach(g=>{
        ...
        const key=[
          ...
          normPlayerV3(name),
          canon(team),
          normMinuteV3(g.minute||g.time||g.clock)||"?"
        ].join("|");
        if(seen.has(key))return;
        seen.add(key);

        const pkey=normPlayerV3(name);
...
function aggregateTeamStatsV3(team){
  const out={
...
function v3Card(title,value,sub,details,icon="📊"){
  return `<div class="v3-card" onclick="toggleV3Card(this)">
    <div class="v3-head">
      <div>
...
function rowsV3(items){
  return items.map(x=>`<div class="v3-row"><span>${x[0]}</span><strong>${x[1]}</strong></div>`).join("");
}

function renderBrasilV3(){
  const s=aggregateTeamStatsV3("Brazil");
  const cards=tournamentTeamCardsV3("Brazil");
  const scorers=goalsByPlayerForTeamV3("Brazil");
  ...
  const scorersRows=scorers.length
    ? rowsV3(scorers.map(x=>[x.name,`${x.goals} gol${x.goals===1?"":"s"}`]))
    : '<div class="v3-muted">Sem gols identificados</div>';

  const discRows=discipline.length
    ? rowsV3(discipline.map(x=>[x.name,`${x.yellow} 🟨 · ${x.red} 🟥`]))
  ...
  return `
    ...
    <div class="v3-grid">
      ${v3Card("Jogos",s.games,`${s.wins}V · ${s.draws}E · ${s.losses}D`,
        rowsV3([["Aproveitamento",`${s.performance.toFixed(0)}%`],["Pontos",s.points],["Saldo",s.gd>=0?`+${s.gd}`:s.gd]]),"🏟️")}
      ...
      ${v3Card("Defesa",s.ga,`${s.cleanSheets} jogo${s.cleanSheets===1?"":"s"} sem sofrer`,
        rowsV3([["Gols sofridos",s.ga],["Saldo",s.gd],["Clean sheets",s.cleanSheets]]),"🛡️")}
      ${v3Card("Chutes",s.shots,s.shotAccuracy!==null?`${s.shotAccuracy.toFixed(1)}% no alvo`:"dados parciais",
        rowsV3([["Chutes",s.shots],["No alvo",s.shotsOnGoal],["Conversão",s.conversion!==null?`${s.conversion.toFixed(1)}%`:"—"]]),"🎯")}
      ${v3Card("Posse média",s.possessionAvg!==null?`${s.possessionAvg.toFixed(1)}%`:"—","média no torneio",
        rowsV3([["Posse média",s.possessionAvg!==null?`${s.possessionAvg.toFixed(1)}%`:"—"],["Precisão de passes",s.passAccuracyAvg!==null?`${s.passAccuracyAvg.toFixed(1)}%`:"—"],["Passes",s.passes||"—"]]),"🧠")}
      ${v3Card("Escanteios",s.corners,"total no torneio",
        rowsV3([["Escanteios",s.corners],["Faltas",s.fouls],["Impedimentos",s.offsides]]),"🚩")}
      ${v3Card("Disciplina",cards.yellow,`${cards.red} expulsão${cards.red===1?"":"ões"}`,
        rowsV3([["Amarelos",cards.yellow],["Vermelhos",cards.red],["Média/jogo",s.games?`${(cards.total/s.games).toFixed(2)}`:"—"]]),"⚖️")}
    </div>
...
function renderStatsV3(){
  ...
  const teamRows=teams.map(team=>{
    const st=aggregateTeamStatsV3(team);
    const cd=tournamentTeamCardsV3(team);
    return {team,...st,yellow:cd.yellow,red:cd.red,totalCards:cd.total};
  });

  const scorersMap=new Map();
  teams.forEach(team=>{
    goalsByPlayerForTeamV3(team).forEach(p=>{
      const k=normPlayerV3(p.name)+"|"+canon(team);
  ...
  return `
    ...
    <div class="v3-grid">
      ...
      ${v3Card("Maior posse",topPoss[0]?.possessionAvg!==null&&topPoss[0]?`${topPoss[0].possessionAvg.toFixed(1)}%`:"—",topPoss[0]?pt(topPoss[0].team):"dados parciais",
        rowsV3(topPoss.map(x=>[`${fl(x.team)} ${pt(x.team)}`,`${x.possessionAvg.toFixed(1)}%`])),"🧠")}
      ${v3Card("Mais chutes",Math.max(...teamRows.map(x=>x.shots),0),
        ...
        rowsV3([...teamRows].sort((a,b)=>b.shots-a.shots).slice(0,10).map(x=>[`${fl(x.team)} ${pt(x.team)}`,x.shots])),"🎯")}
      ${v3Card("Mais faltas",Math.max(...teamRows.map(x=>x.fouls),0),
        ...
        rowsV3([...teamRows].sort((a,b)=>b.fouls-a.fouls).slice(0,10).map(x=>[`${fl(x.team)} ${pt(x.team)}`,x.fouls])),"⚖️")}
    </div>
...
renderBrasil = renderBrasilV3;
renderStats = renderStatsV3;


// ============================
// V3 - ESTATÍSTICAS RÁPIDAS E CORRIGIDAS
// ============================

let STATS_CACHE_V3={key:"",model:null,building:false};

function invalidateStatsCacheV3(){
  STATS_CACHE_V3={key:"",model:null,building:false};
}

function statsFingerprintV3(){
...
function statSideV3(stats,key,side){
  const map={
    shotsOnGoal:"shotsOnTarget",
    passes:"totalPasses"
  };
  const sourceKey=map[key]||key;
  return numberV3(stats?.[sourceKey]?.[side]);
}

function simpleCardEventsV3(m,side){
  const team=side==="home"?m.h:m.a;
  const raw=[];
  const ofb=ofbMatch(m.h,m.a);
  ...
  raw.forEach(c=>{
    ...
    const playerKey=normPlayerV3(name);
    const type=cardType(c);
    const minute=normMinuteV3(c.minute||c.time||c.clock);
...
function buildStatsModelV3(){
  ...
  const byTeam=new Map(teams.map(t=>[t,emptyTeamV3(t)]));
  ...
  const getTeam=name=>{
    ...
    if(!exact&&name&&!isPlaceholderTeamV23(name)){
      exact=name;
      teams.push(name);
      byTeam.set(name,emptyTeamV3(name));
    }
  ...
  F.filter(m=>mSt(m)==="finished"&&!isPlaceholderTeamV23(m.h)&&!isPlaceholderTeamV23(m.a))
   .forEach(m=>{
    ...
    [["home",home],["away",away]].forEach(([side,t])=>{
      ...
      const values={
        shots:statSideV3(stats,"shots",side),
        shotsOnGoal:statSideV3(stats,"shotsOnGoal",side),
        corners:statSideV3(stats,"corners",side),
        fouls:statSideV3(stats,"fouls",side),
        offsides:statSideV3(stats,"offsides",side),
        passes:statSideV3(stats,"passes",side),
        possession:statSideV3(stats,"possession",side),
        passAccuracy:statSideV3(stats,"passAccuracy",side)
      };
    ...
    (allGoalsForMatchV3(m,ofb)||[])
      ...
      .forEach(g=>{
        ...
        const playerKey=normPlayerV3(name)+"|"+canon(team);
...
function getStatsModelV3(){
  const key=statsFingerprintV3();
  if(STATS_CACHE_V3.model&&STATS_CACHE_V3.key===key){
    return STATS_CACHE_V3.model;
  }

  const model=buildStatsModelV3();
  STATS_CACHE_V3={key,model,building:false};
  return model;
}

function prewarmStatsCacheV3(){
  setTimeout(()=>{
    try{getStatsModelV3()}catch(e){console.warn("V3 stats cache",e)}
  },80);
}

function teamModelV3(team){
  const model=getStatsModelV3();
  return model.teams.find(x=>nm(x.team,team))||emptyTeamV3(team);
}

function valueOrDashV3(v,suffix=""){
  return v===null||v===undefined||Number.isNaN(v)?"—":`${v}${suffix}`;
}

function renderBrasilFastV3(){
  const s=teamModelV3("Brazil");
  const model=getStatsModelV3();
  ...
  return `
    ...
    <div class="v3-grid">
      ...
      ${v3Card("Defesa",s.ga,`${s.cleanSheets} jogo${s.cleanSheets===1?"":"s"} sem sofrer`,
        rowsV3([["Gols sofridos",s.ga],["Saldo",s.gd],["Jogos sem sofrer",s.cleanSheets]]),"🛡️")}

      ${v3Card("Chutes",s.statsGames?valueOrDashV3(s.shots):"—",
        s.statsGames&&s.shotAccuracy!==null?`${s.shotAccuracy.toFixed(1)}% no alvo`:"dado ainda não disponível",
        rowsV3([["Chutes",s.statsGames?s.shots:"—"],["No alvo",s.statsGames?s.shotsOnGoal:"—"],["Conversão",s.conversion!==null?`${s.conversion.toFixed(1)}%`:"—"]]),"🎯")}
      ...
      ${v3Card("Posse média",s.possessionAvg!==null?`${s.possessionAvg.toFixed(1)}%`:"—",
        ...
        rowsV3([["Posse média",s.possessionAvg!==null?`${s.possessionAvg.toFixed(1)}%`:"—"],["Precisão de passes",s.passAccuracyAvg!==null?`${s.passAccuracyAvg.toFixed(1)}%`:"—"],["Passes",s.statsGames?s.passes:"—"]]),"🧠")}

      ${v3Card("Escanteios",s.statsGames?s.corners:"—",
        s.statsGames?"total no torneio":"dado ainda não disponível",
        rowsV3([["Escanteios",s.statsGames?s.corners:"—"],["Faltas",s.statsGames?s.fouls:"—"],["Impedimentos",s.statsGames?s.offsides:"—"]]),"🚩")}

      ${v3Card("Disciplina",s.yellow,`${s.red} expulsão${s.red===1?"":"ões"}`,
        rowsV3([["Amarelos",s.yellow],["Vermelhos",s.red],["Média por jogo",s.games?(s.totalCards/s.games).toFixed(2):"—"]]),"⚖️")}
    </div>
  ...
  return `
    ...
    <div class="v3-grid">
      ...
      ${v3Card("Artilheiros",model.scorers[0]?.goals||"—",
        ...
        rowsV3(model.scorers.slice(0,20).map(x=>[`${x.name} · ${pt(x.team)}`,x.goals])),"🥇")}

      ${v3Card("Cartões",totalYellow+totalRed,
        `${totalYellow} amarelos · ${totalRed} vermelhos`,
        topCards.length?rowsV3(topCards.map(x=>[`${fl(x.team)} ${pt(x.team)}`,`${x.yellow} 🟨 · ${x.red} 🟥`])):'<div class="v3-muted">Dados ainda não disponíveis</div>',"🟨")}
      ...
      ${v3Card("Maior posse",topPoss[0]?`${topPoss[0].possessionAvg.toFixed(1)}%`:"—",
        ...
        topPoss.length?rowsV3(topPoss.map(x=>[`${fl(x.team)} ${pt(x.team)}`,`${x.possessionAvg.toFixed(1)}%`])):'<div class="v3-muted">A fonte ainda não entregou posse consolidada</div>',"🧠")}

      ${v3Card("Mais chutes",topShots[0]?.shots??"—",
        topShots[0]?pt(topShots[0].team):"dado ainda não disponível",
        topShots.length?rowsV3(topShots.map(x=>[`${fl(x.team)} ${pt(x.team)}`,x.shots])):'<div class="v3-muted">A fonte ainda não entregou chutes consolidados</div>',"🎯")}

      ${v3Card("Mais faltas",topFouls[0]?.fouls??"—",
        topFouls[0]?pt(topFouls[0].team):"dado ainda não disponível",
        topFouls.length?rowsV3(topFouls.map(x=>[`${fl(x.team)} ${pt(x.team)}`,x.fouls])):'<div class="v3-muted">A fonte ainda não entregou faltas consolidadas</div>',"⚖️")}
    </div>
...
renderBrasil=renderBrasilFastV3;
renderStats=renderStatsFastV3;


// ============================
// V3 - MELHORES TERCEIROS NO MATA-MATA
// ============================

function groupThirdsV3(){
  const out=[];
...
function bestThirdTeamsV3(){
  return groupThirdsV3().slice(0,8);
}

function isBestThirdSlotV3(slot){
  return /3º colocado|melhor\s*3/i.test(String(slot||""));
}

function applyBestThirdsV3(){
  const thirds=bestThirdTeamsV3();
  ...
  F.filter(m=>/^e\d+$/i.test(m.id)).forEach(m=>{
    [["h","slotH"],["a","slotA"]].forEach(([side,slotKey])=>{
      ...
      if(!isBestThirdSlotV3(slot))return;
      if(!isPlaceholderTeamV23(m[side]) && !isBestThirdSlotV3(m[side]))return;
...
const resolveKnockoutBaseV3 = resolveKnockoutV23;
resolveKnockoutV23 = function(){
  resolveKnockoutBaseV3();
  applyBestThirdsV3();
  applyOfficialParticipantsV23();
...
// ============================
// V3 - PLACAR COM PRORROGAÇÃO E PÊNALTIS
// ============================
...
function penaltyInfoV3(m){
  ...
  const hp=readPenaltyScoreV3(home);
  const ap=readPenaltyScoreV3(away);
...
function statusIsPenaltyV3(m,data){
  const txt=[
    data?.min,
    espnStatusFor(m)?.min,
...
const winnerLoserBaseV3 = winnerLoserV23;
winnerLoserV23 = function(m){
  const base=winnerLoserBaseV3(m);
  if(base)return base;

  const pk=penaltyInfoV3(m);
  if(pk)return {winner:pk.winner,loser:pk.loser};

  return null;
};

const mkCardBaseV3 = mkCard;
mkCard = function(m){
  const data=mData(m);
  ...
  let pill="";
  if(st==="live"){
    const pen=statusIsPenaltyV3(m,data);
  ...
  let goalsSum="";
  if(st!=="upcoming")goalsSum=goalsSummaryV3(m);
...
// V3 - ESTÁVEL
// Correções pontuais:
// 1) casamento rigoroso de jogo por data + seleções;
// 2) aliases PT-BR/EN para evitar placar zerado;
// 3) deduplicação e limite de gols conforme placar.
// ============================

function teamKeyV3Stable(name){
  const c=canon(name);
  const map={
    "alemanha":"germany","germany":"germany",
    "paraguai":"paraguay","paraguay":"paraguay",
    "paises baixos":"netherlands","holanda":"netherlands","netherlands":"netherlands",
    "marrocos":"morocco","morocco":"morocco",
    "costa do marfim":"ivory coast","cote d ivoire":"ivory coast","côte d ivoire":"ivory coast","ivory coast":"ivory coast",
    ...
    "coreia do sul":"south korea","south korea":"south korea","korea republic":"south korea",
    "tchequia":"czechia","chequia":"czechia","czechia":"czechia","czech republic":"czechia",
    "turquia":"turkey","turkiye":"turkey","türkiye":"turkey","turkey":"turkey",
...
const sameTeamBaseV3Stable = sameTeamV16;
sameTeamV16 = function(a,b){
  if(teamKeyV3Stable(a)===teamKeyV3Stable(b))return true;
  return sameTeamBaseV3Stable(a,b);
};

function sameFixtureTeamsV3Stable(ev,m){
  const t=espnTeamsForEventV23(ev);
  return (sameTeamV16(t.home,m.h)&&sameTeamV16(t.away,m.a)) ||
         (sameTeamV16(t.home,m.a)&&sameTeamV16(t.away,m.h));
}

function sameFixtureDateV3Stable(ev,m){
  const br=utcBRT(ev.date||ev.competitions?.[0]?.date);
  return br.date===m.d;
}

function strictEventForMatchV3Stable(m){
  if(isPlaceholderTeamV23(m.h)||isPlaceholderTeamV23(m.a))return null;
  return ESPN_EVENTS.find(ev=>sameFixtureDateV3Stable(ev,m)&&sameFixtureTeamsV3Stable(ev,m))||null;
}

const espnEventForBaseV3Stable = espnEventFor;
espnEventFor = function(m){
  const strict=strictEventForMatchV3Stable(m);
  if(strict)return strict;
  // Não usar evento aproximado com seleções reais; evita placar em jogo errado.
  if(!isPlaceholderTeamV23(m.h)&&!isPlaceholderTeamV23(m.a))return null;
  return null;
};

const espnEventByFixtureBaseV3Stable = espnEventByFixtureV23;
espnEventByFixtureV23 = function(m){
  const strict=strictEventForMatchV3Stable(m);
  if(strict)return strict;

  // Placeholder futuro não deve herdar placar de jogo próximo.
  if(mSt(m)==="upcoming")return null;

  // Só permite aproximação quando a partida já passou e ainda há placeholder.
  if(isPlaceholderTeamV23(m.h)||isPlaceholderTeamV23(m.a))return espnEventByFixtureBaseV3Stable(m);

  return null;
};

function normalizeGoalMinuteV3Stable(v){
  const s=String(v??"").trim();
  if(!s)return "?";
  const plus=s.match(/(\d+)\s*\+\s*(\d+)/);
...
function normalizeGoalNameV3Stable(name){
  return canon(String(name||"").replace(/\([^)]*\)/g," ").replace(/\s+/g," ").trim());
}

function goalDedupeV3Stable(goals,m){
  const seen=new Set();
  const out=[];
  (goals||[]).forEach(g=>{
    const name=String(g.name||g.player||"").trim();
    if(!name)return;
    const side=g.side || (sameTeamV16(g.team,m.h)?"home":sameTeamV16(g.team,m.a)?"away":"");
    const key=[
      m.id,
      normalizeGoalNameV3Stable(name),
      side||teamKeyV3Stable(g.team),
      normalizeGoalMinuteV3Stable(g.minute||g.time||g.clock||g.displayClock)
    ].join("|");
    if(seen.has(key))return;
    seen.add(key);
    out.push({...g,side});
  ...
  // Se ainda houver gols em excesso, respeita o placar oficial.
  const d=mData(m);
  if(d&&d.hasScore){
    const capHome=Number(d.hs), capAway=Number(d.as);
    if(Number.isFinite(capHome)&&Number.isFinite(capAway)){
      const h=out.filter(g=>g.side==="home").sort((a,b)=>parseInt(a.minute||999)-parseInt(b.minute||999)).slice(0,capHome);
      const a=out.filter(g=>g.side==="away").sort((a,b)=>parseInt(a.minute||999)-parseInt(b.minute||999)).slice(0,capAway);
      const other=out.filter(g=>g.side!=="home"&&g.side!=="away");
      return [...h,...a,...other].sort((x,y)=>parseInt(x.minute||999)-parseInt(y.minute||999));
    }
  }

  return out.sort((a,b)=>parseInt(a.minute||999)-parseInt(b.minute||999));
}

const allGoalsForMatchBaseV3Stable = allGoalsForMatchV3;
allGoalsForMatchV3 = function(m,ofb){
  return goalDedupeV3Stable(allGoalsForMatchBaseV3Stable(m,ofb),m);
};

goalsSummaryV3 = function(m){
  const ofb=ofbMatch(m.h,m.a);
  const goals=goalDedupeV3Stable(allGoalsForMatchBaseV3Stable(m,ofb),m);
  if(!goals.length)return "";
  const h=[],a=[];
  goals.forEach(g=>{
    const txt=`${fl(g.team)} ${g.name}${g.minute?` ${g.minute}'`:""}${g.penalty?" (P)":""}`;
    if(g.side==="home")h.push(txt);
    else if(g.side==="away")a.push(txt);
  });
  return `<div class="goal-line">${[...h,...a].join(" | ")}</div>`;
};


// ============================
// V3 - CORREÇÃO DE PLACAR DE JOGOS JÁ ENCERRADOS
// Mantém proteção contra jogo futuro, mas permite fallback seguro
// para partidas já passadas quando a ESPN usa nomes diferentes.
// ============================

function matchKickTsV3Score(m){
  return matchKick(m).getTime();
}

function isPastEnoughForScoreV3(m){
  // Só permite fallback de placar quando já passou tempo suficiente do início.
  // 2h30 cobre tempo normal, acréscimos e parte de prorrogação.
  return Date.now() > matchKickTsV3Score(m) + (150*60*1000);
}

function espnEventStatusDoneV3(ev){
  const typ=ev?.status?.type || ev?.competitions?.[0]?.status?.type || {};
  return !!(typ.completed || typ.state==="post");
}

function eventTimeDiffMinV3(ev,m){
  const evTs=new Date(ev.date||ev.competitions?.[0]?.date).getTime();
  const mt=matchKickTsV3Score(m);
  if(!Number.isFinite(evTs)||!Number.isFinite(mt))return 9999;
  return Math.abs(evTs-mt)/60000;
}

function eventTeamHitsV3(ev,m){
  const t=espnTeamsForEventV23(ev);
  let hits=0;
...
function safeEventForScoreV3(m){
  if(isPlaceholderTeamV23(m.h)||isPlaceholderTeamV23(m.a))return null;

  const sameDay=ESPN_EVENTS.filter(ev=>{
    const br=utcBRT(ev.date||ev.competitions?.[0]?.date);
    return br.date===m.d;
  });
  ...
  // 1) Melhor caso: os dois times batem.
  const exact=sameDay.find(ev=>eventTeamHitsV3(ev,m)===2);
  if(exact)return exact;

  // 2) Fallback seguro: jogo já passou, evento finalizado,
  // horário muito próximo e pelo menos um time bate.
  if(!isPastEnoughForScoreV3(m))return null;

  const candidates=sameDay
    .map(ev=>({
      ev,
      hits:eventTeamHitsV3(ev,m),
      diff:eventTimeDiffMinV3(ev,m),
      done:espnEventStatusDoneV3(ev)
    }))
    .filter(x=>x.done && x.hits>=1 && x.diff<=35)
    .sort((a,b)=>b.hits-a.hits || a.diff-b.diff);

  return candidates[0]?.ev||null;
}

const espnEventForV3ScoreBase = espnEventFor;
espnEventFor = function(m){
  return safeEventForScoreV3(m);
};

const espnEventByFixtureV3ScoreBase = espnEventByFixtureV23;
espnEventByFixtureV23 = function(m){
  return safeEventForScoreV3(m);
};
...
// ============================
// V3 - CORREÇÃO DO CHAVEAMENTO OFICIAL
// Regra: para mata-mata futuro, participantes oficiais por data + horário
// têm prioridade sobre o cálculo genérico de melhores terceiros.
// ============================

function officialEventByDateTimeV3(m){
  if(m.ph==="grupos")return null;

  const target=matchKick(m).getTime();

  const candidates=ESPN_EVENTS.map(ev=>{
    const evDate=ev.date||ev.competitions?.[0]?.date;
    const evTs=new Date(evDate).getTime();
    const br=utcBRT(evDate);
    return {
      ev,
      sameDate:br.date===m.d,
      diff:Number.isFinite(evTs)&&Number.isFinite(target)?Math.abs(evTs-target)/60000:9999
    };
  })
  .filter(x=>x.sameDate && x.diff<=45)
  .sort((a,b)=>a.diff-b.diff);

  return candidates[0]?.ev||null;
}

function applyOfficialKnockoutParticipantsV3(){
  F.filter(m=>m.ph!=="grupos").forEach(m=>{
    const ev=officialEventByDateTimeV3(m);
    if(!ev)return;

    const t=espnTeamsForEventV23(ev);
    if(!t.home||!t.away)return;

    if(!isPlaceholderTeamV23(t.home))m.h=t.home;
    if(!isPlaceholderTeamV23(t.away))m.a=t.away;
  });
...
const resolveKnockoutBeforeOfficialV3 = resolveKnockoutV23;
resolveKnockoutV23 = function(){
  resolveKnockoutBeforeOfficialV3();
  applyOfficialKnockoutParticipantsV3();
};

const renderBeforeOfficialBracketV3 = render;
render = function(){
  applyOfficialKnockoutParticipantsV3();
  return renderBeforeOfficialBracketV3();
};


// ============================
// V3 - MATA-MATA OFICIAL DE SEGURANÇA
// Fonte de segurança para confrontos já definidos do mata-mata.
// Evita que o cálculo genérico de melhores terceiros troque adversários
// e garante placar para jogos finalizados quando a API não casa nomes.
// ============================

const OFFICIAL_KO_V3 = {
...
function applyOfficialKOSnapshotV3(){
  Object.entries(OFFICIAL_KO_V3).forEach(([id,o])=>{
...
function officialKODataV3(m){
  const o=OFFICIAL_KO_V3[m.id];
  if(!o)return null;
  ...
  // Para futuros, protege contra placar herdado de outro jogo.
...
const resolveKnockoutBeforeSnapshotV3 = resolveKnockoutV23;
resolveKnockoutV23 = function(){
  resolveKnockoutBeforeSnapshotV3();
  applyOfficialKOSnapshotV3();
};

const renderBeforeSnapshotV3 = render;
render = function(){
  applyOfficialKOSnapshotV3();
  return renderBeforeSnapshotV3();
};

const mDataBeforeSnapshotV3 = mData;
mData = function(m){
  const official=officialKODataV3(m);
  if(official)return official;
  return mDataBeforeSnapshotV3(m);
};

const penaltyInfoBeforeSnapshotV3 = typeof penaltyInfoV3 === "function" ? penaltyInfoV3 : null;
if(penaltyInfoBeforeSnapshotV3){
  penaltyInfoV3 = function(m){
    const o=OFFICIAL_KO_V3[m.id];
    ...
    return penaltyInfoBeforeSnapshotV3(m);
  };
}

// Garante que gols antigos de adversário errado não apareçam em jogos corrigidos.
const goalsSummaryBeforeSnapshotV3 = goalsSummaryV9;
goalsSummaryV9 = function(m){
  const o=OFFICIAL_KO_V3[m.id];
  if(o&&o.st==="finished"){
    // Sem inventar artilheiros. Se a fonte não casar corretamente,
    // mostra o placar e deixa gols detalhados apenas no modal quando confiáveis.
    const raw=goalsSummaryBeforeSnapshotV3(m);
    if(raw && !raw.includes("Bósnia") && !raw.includes("Bosnia") && !raw.includes("Bosnia-Herz")) return raw;
    return "";
  }
  return goalsSummaryBeforeSnapshotV3(m);
};

console.log('Copa 2026 V3 carregado');
