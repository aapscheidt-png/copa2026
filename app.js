...
const ESPN_CACHE_KEY_V22="copa2026_data_v6";
...
function renderStats(){
  ...
  return`<div class="stats-version">✓ V6"kpi-grid">
...
<table class="info-tbl">
  ...
  <tr><td>Versão</td><td style="color:var(--gold)">V6</td></tr>
</table></div>`;
}
...
function renderBrasil(){
  ...
  return `<div class="pro-badge">✓ V6"team-hero">
...
async function loadAll(){
  ...
  resolveKnockoutV23();
  if(typeof invalidateStatsCacheV6==="function")invalidateStatsCacheV6();
  render();
  if(typeof prewarmStatsCacheV6==="function")prewarmStatsCacheV6();
...
// ============================
// V6 - CAMADA ESTATÍSTICA NORMALIZADA
// ============================

function normMinuteV6(v){
...
function cardEventKeyV6(m,c){
  const eventId=c.id||c.eventId||c.uid||c.guid||"";
  if(eventId)return `${m.id}|id:${eventId}`;
  return [
    m.id,
    normPlayerV6(c.name||c.player||c.athlete||c.displayName||""),
    canon(c.team||""),
    cardType(c),
    normMinuteV6(c.minute||c.time||c.clock||"?")??"?"
  ].join("|");
}

function normalizeCardEventsV6(m,side){
  const team=side==="home"?m.h:m.a;
  ...
  raw.forEach(c=>{
    ...
    const type=cardType(c);
    const key=cardEventKeyV6(m,{...c,team});
    if(!map.has(key)){
      map.set(key,{
        id:c.id||c.eventId||null,
        name,
        playerKey:normPlayerV6(name),
        team,
        minute:normMinuteV6(c.minute||c.time||c.clock),
        card:type,
...
function teamOfficialCardTotalsV6(m,side){
...
function teamCardsForMatchV6(m,side){
  const events=normalizeCardEventsV6(m,side);
  const official=teamOfficialCardTotalsV6(m,side);
  ...
  // Regra V6: se houver total oficial, ele é soberano.
...
function tournamentTeamCardsV6(team){
  let yellow=0,red=0;
  const players=new Map();

  F.filter(m=>mSt(m)!=="upcoming"&&(nm(m.h,team)||nm(m.a,team))).forEach(m=>{
    const side=nm(m.h,team)?"home":"away";
    const pack=teamCardsForMatchV6(m,side);
    yellow+=pack.yellow;
    red+=pack.red;
...
function goalsByPlayerForTeamV6(team){
  const map=new Map();
  const seen=new Set();
    ...
    (allGoalsForMatchV9(m,ofb)||[])
      ...
      .forEach(g=>{
        ...
        const key=[
          ...
          normPlayerV6(name),
          canon(team),
          normMinuteV6(g.minute||g.time||g.clock)||"?"
        ].join("|");
        if(seen.has(key))return;
        seen.add(key);

        const pkey=normPlayerV6(name);
...
function aggregateTeamStatsV6(team){
  const out={
...
function v3Card(title,value,sub,details,icon="📊"){
  return `<div class="v3-card" onclick="toggleV6Card(this)">
    <div class="v3-head">
      <div>
...
function rowsV6(items){
  return items.map(x=>`<div class="v3-row"><span>${x[0]}</span><strong>${x[1]}</strong></div>`).join("");
}

function renderBrasilV6(){
  const s=aggregateTeamStatsV6("Brazil");
  const cards=tournamentTeamCardsV6("Brazil");
  const scorers=goalsByPlayerForTeamV6("Brazil");
  ...
  const scorersRows=scorers.length
    ? rowsV6(scorers.map(x=>[x.name,`${x.goals} gol${x.goals===1?"":"s"}`]))
    : '<div class="v3-muted">Sem gols identificados</div>';

  const discRows=discipline.length
    ? rowsV6(discipline.map(x=>[x.name,`${x.yellow} 🟨 · ${x.red} 🟥`]))
  ...
  return `
    ...
    <div class="v3-grid">
      ${v3Card("Jogos",s.games,`${s.wins}V · ${s.draws}E · ${s.losses}D`,
        rowsV6([["Aproveitamento",`${s.performance.toFixed(0)}%`],["Pontos",s.points],["Saldo",s.gd>=0?`+${s.gd}`:s.gd]]),"🏟️")}
      ...
      ${v3Card("Defesa",s.ga,`${s.cleanSheets} jogo${s.cleanSheets===1?"":"s"} sem sofrer`,
        rowsV6([["Gols sofridos",s.ga],["Saldo",s.gd],["Clean sheets",s.cleanSheets]]),"🛡️")}
      ${v3Card("Chutes",s.shots,s.shotAccuracy!==null?`${s.shotAccuracy.toFixed(1)}% no alvo`:"dados parciais",
        rowsV6([["Chutes",s.shots],["No alvo",s.shotsOnGoal],["Conversão",s.conversion!==null?`${s.conversion.toFixed(1)}%`:"—"]]),"🎯")}
      ${v3Card("Posse média",s.possessionAvg!==null?`${s.possessionAvg.toFixed(1)}%`:"—","média no torneio",
        rowsV6([["Posse média",s.possessionAvg!==null?`${s.possessionAvg.toFixed(1)}%`:"—"],["Precisão de passes",s.passAccuracyAvg!==null?`${s.passAccuracyAvg.toFixed(1)}%`:"—"],["Passes",s.passes||"—"]]),"🧠")}
      ${v3Card("Escanteios",s.corners,"total no torneio",
        rowsV6([["Escanteios",s.corners],["Faltas",s.fouls],["Impedimentos",s.offsides]]),"🚩")}
      ${v3Card("Disciplina",cards.yellow,`${cards.red} expulsão${cards.red===1?"":"ões"}`,
        rowsV6([["Amarelos",cards.yellow],["Vermelhos",cards.red],["Média/jogo",s.games?`${(cards.total/s.games).toFixed(2)}`:"—"]]),"⚖️")}
    </div>
...
function renderStatsV6(){
  ...
  const teamRows=teams.map(team=>{
    const st=aggregateTeamStatsV6(team);
    const cd=tournamentTeamCardsV6(team);
    return {team,...st,yellow:cd.yellow,red:cd.red,totalCards:cd.total};
  });

  const scorersMap=new Map();
  teams.forEach(team=>{
    goalsByPlayerForTeamV6(team).forEach(p=>{
      const k=normPlayerV6(p.name)+"|"+canon(team);
  ...
  return `
    ...
    <div class="v3-grid">
      ...
      ${v3Card("Maior posse",topPoss[0]?.possessionAvg!==null&&topPoss[0]?`${topPoss[0].possessionAvg.toFixed(1)}%`:"—",topPoss[0]?pt(topPoss[0].team):"dados parciais",
        rowsV6(topPoss.map(x=>[`${fl(x.team)} ${pt(x.team)}`,`${x.possessionAvg.toFixed(1)}%`])),"🧠")}
      ${v3Card("Mais chutes",Math.max(...teamRows.map(x=>x.shots),0),
        ...
        rowsV6([...teamRows].sort((a,b)=>b.shots-a.shots).slice(0,10).map(x=>[`${fl(x.team)} ${pt(x.team)}`,x.shots])),"🎯")}
      ${v3Card("Mais faltas",Math.max(...teamRows.map(x=>x.fouls),0),
        ...
        rowsV6([...teamRows].sort((a,b)=>b.fouls-a.fouls).slice(0,10).map(x=>[`${fl(x.team)} ${pt(x.team)}`,x.fouls])),"⚖️")}
    </div>
...
renderBrasil = renderBrasilV6;
renderStats = renderStatsV6;


// ============================
// V6 - ESTATÍSTICAS RÁPIDAS E CORRIGIDAS
// ============================

let STATS_CACHE_V6={key:"",model:null,building:false};

function invalidateStatsCacheV6(){
  STATS_CACHE_V6={key:"",model:null,building:false};
}

function statsFingerprintV6(){
...
function statSideV6(stats,key,side){
  const map={
    shotsOnGoal:"shotsOnTarget",
    passes:"totalPasses"
  };
  const sourceKey=map[key]||key;
  return numberV6(stats?.[sourceKey]?.[side]);
}

function simpleCardEventsV6(m,side){
  const team=side==="home"?m.h:m.a;
  const raw=[];
  const ofb=ofbMatch(m.h,m.a);
  ...
  raw.forEach(c=>{
    ...
    const playerKey=normPlayerV6(name);
    const type=cardType(c);
    const minute=normMinuteV6(c.minute||c.time||c.clock);
...
function buildStatsModelV6(){
  ...
  const byTeam=new Map(teams.map(t=>[t,emptyTeamV6(t)]));
  ...
  const getTeam=name=>{
    ...
    if(!exact&&name&&!isPlaceholderTeamV23(name)){
      exact=name;
      teams.push(name);
      byTeam.set(name,emptyTeamV6(name));
    }
  ...
  F.filter(m=>mSt(m)==="finished"&&!isPlaceholderTeamV23(m.h)&&!isPlaceholderTeamV23(m.a))
   .forEach(m=>{
    ...
    [["home",home],["away",away]].forEach(([side,t])=>{
      ...
      const values={
        shots:statSideV6(stats,"shots",side),
        shotsOnGoal:statSideV6(stats,"shotsOnGoal",side),
        corners:statSideV6(stats,"corners",side),
        fouls:statSideV6(stats,"fouls",side),
        offsides:statSideV6(stats,"offsides",side),
        passes:statSideV6(stats,"passes",side),
        possession:statSideV6(stats,"possession",side),
        passAccuracy:statSideV6(stats,"passAccuracy",side)
      };
    ...
    (allGoalsForMatchV9(m,ofb)||[])
      ...
      .forEach(g=>{
        ...
        const playerKey=normPlayerV6(name)+"|"+canon(team);
...
function getStatsModelV6(){
  const key=statsFingerprintV6();
  if(STATS_CACHE_V6.model&&STATS_CACHE_V6.key===key){
    return STATS_CACHE_V6.model;
  }

  const model=buildStatsModelV6();
  STATS_CACHE_V6={key,model,building:false};
  return model;
}

function prewarmStatsCacheV6(){
  setTimeout(()=>{
    try{getStatsModelV6()}catch(e){console.warn("V6 stats cache",e)}
  },80);
}

function teamModelV6(team){
  const model=getStatsModelV6();
  return model.teams.find(x=>nm(x.team,team))||emptyTeamV6(team);
}

function valueOrDashV6(v,suffix=""){
  return v===null||v===undefined||Number.isNaN(v)?"—":`${v}${suffix}`;
}

function renderBrasilFastV6(){
  const s=teamModelV6("Brazil");
  const model=getStatsModelV6();
  ...
  return `
    ...
    <div class="v3-grid">
      ...
      ${v3Card("Defesa",s.ga,`${s.cleanSheets} jogo${s.cleanSheets===1?"":"s"} sem sofrer`,
        rowsV6([["Gols sofridos",s.ga],["Saldo",s.gd],["Jogos sem sofrer",s.cleanSheets]]),"🛡️")}

      ${v3Card("Chutes",s.statsGames?valueOrDashV6(s.shots):"—",
        s.statsGames&&s.shotAccuracy!==null?`${s.shotAccuracy.toFixed(1)}% no alvo`:"dado ainda não disponível",
        rowsV6([["Chutes",s.statsGames?s.shots:"—"],["No alvo",s.statsGames?s.shotsOnGoal:"—"],["Conversão",s.conversion!==null?`${s.conversion.toFixed(1)}%`:"—"]]),"🎯")}
      ...
      ${v3Card("Posse média",s.possessionAvg!==null?`${s.possessionAvg.toFixed(1)}%`:"—",
        ...
        rowsV6([["Posse média",s.possessionAvg!==null?`${s.possessionAvg.toFixed(1)}%`:"—"],["Precisão de passes",s.passAccuracyAvg!==null?`${s.passAccuracyAvg.toFixed(1)}%`:"—"],["Passes",s.statsGames?s.passes:"—"]]),"🧠")}

      ${v3Card("Escanteios",s.statsGames?s.corners:"—",
        s.statsGames?"total no torneio":"dado ainda não disponível",
        rowsV6([["Escanteios",s.statsGames?s.corners:"—"],["Faltas",s.statsGames?s.fouls:"—"],["Impedimentos",s.statsGames?s.offsides:"—"]]),"🚩")}

      ${v3Card("Disciplina",s.yellow,`${s.red} expulsão${s.red===1?"":"ões"}`,
        rowsV6([["Amarelos",s.yellow],["Vermelhos",s.red],["Média por jogo",s.games?(s.totalCards/s.games).toFixed(2):"—"]]),"⚖️")}
    </div>
  ...
  return `
    ...
    <div class="v3-grid">
      ...
      ${v3Card("Artilheiros",model.scorers[0]?.goals||"—",
        ...
        rowsV6(model.scorers.slice(0,20).map(x=>[`${x.name} · ${pt(x.team)}`,x.goals])),"🥇")}

      ${v3Card("Cartões",totalYellow+totalRed,
        `${totalYellow} amarelos · ${totalRed} vermelhos`,
        topCards.length?rowsV6(topCards.map(x=>[`${fl(x.team)} ${pt(x.team)}`,`${x.yellow} 🟨 · ${x.red} 🟥`])):'<div class="v3-muted">Dados ainda não disponíveis</div>',"🟨")}
      ...
      ${v3Card("Maior posse",topPoss[0]?`${topPoss[0].possessionAvg.toFixed(1)}%`:"—",
        ...
        topPoss.length?rowsV6(topPoss.map(x=>[`${fl(x.team)} ${pt(x.team)}`,`${x.possessionAvg.toFixed(1)}%`])):'<div class="v3-muted">A fonte ainda não entregou posse consolidada</div>',"🧠")}

      ${v3Card("Mais chutes",topShots[0]?.shots??"—",
        topShots[0]?pt(topShots[0].team):"dado ainda não disponível",
        topShots.length?rowsV6(topShots.map(x=>[`${fl(x.team)} ${pt(x.team)}`,x.shots])):'<div class="v3-muted">A fonte ainda não entregou chutes consolidados</div>',"🎯")}

      ${v3Card("Mais faltas",topFouls[0]?.fouls??"—",
        topFouls[0]?pt(topFouls[0].team):"dado ainda não disponível",
        topFouls.length?rowsV6(topFouls.map(x=>[`${fl(x.team)} ${pt(x.team)}`,x.fouls])):'<div class="v3-muted">A fonte ainda não entregou faltas consolidadas</div>',"⚖️")}
    </div>
...
renderBrasil=renderBrasilFastV6;
renderStats=renderStatsFastV6;


// ============================
// V6 - MELHORES TERCEIROS NO MATA-MATA
// ============================

function groupThirdsV6(){
  const out=[];
...
function bestThirdTeamsV6(){
  return groupThirdsV6().slice(0,8);
}

function isBestThirdSlotV6(slot){
  return /3º colocado|melhor\s*3/i.test(String(slot||""));
}

function applyBestThirdsV6(){
  const thirds=bestThirdTeamsV6();
  ...
  F.filter(m=>/^e\d+$/i.test(m.id)).forEach(m=>{
    [["h","slotH"],["a","slotA"]].forEach(([side,slotKey])=>{
      ...
      if(!isBestThirdSlotV6(slot))return;
      if(!isPlaceholderTeamV23(m[side]) && !isBestThirdSlotV6(m[side]))return;
...
const resolveKnockoutBaseV6 = resolveKnockoutV23;
resolveKnockoutV23 = function(){
  resolveKnockoutBaseV6();
  applyBestThirdsV6();
  applyOfficialParticipantsV23();
...
console.log('Copa 2026 V6 carregado');
