...
// APIs: football-data.org + TheSportsDB
// ═══════════════════════════════════════════
...
// ───────────────────────────────────────────
// DICIONÁRIOS — bandeiras e nomes em PT
// ───────────────────────────────────────────
const FL = {
  ...
  "United States":"🇺🇸","Paraguay":"🇵🇾","Germany":"🇩🇪","Curacao":"🇨🇼",
  "Netherlands":"🇳🇱","Japan":"🇯🇵","Ivory Coast":"🇨🇮","Côte d'Ivoire":"🇨🇮",
  "Ecuador":"🇪🇨","Sweden":"🇸🇪","Tunisia":"🇹🇳","Spain":"🇪🇸",
  "Cape Verde":"🇨🇻","Belgium":"🇧🇪","Egypt":"🇪🇬","Saudi Arabia":"🇸🇦",
  "Uruguay":"🇺🇾","Iran":"🇮🇷","New Zealand":"🇳🇿","Austria":"🇦🇹",
  "Jordan":"🇯🇴","France":"🇫🇷","Senegal":"🇸🇳","Iraq":"🇮🇶",
  "Norway":"🇳🇴","Argentina":"🇦🇷","Algeria":"🇩🇿","Portugal":"🇵🇹",
  "DR Congo":"🇨🇩","England":"🏴󠁧󠁢󠁥󠁮󠁧󠁿","Croatia":"🇭🇷","Ghana":"🇬🇭",
  "Panama":"🇵🇦","Uzbekistan":"🇺🇿","Colombia":"🇨🇴","Korea Republic":"🇰🇷"
};
const PT = {
  ...
  "United States":"EUA","Paraguay":"Paraguai","Germany":"Alemanha",
  "Curacao":"Curaçao","Netherlands":"Países Baixos","Japan":"Japão",
  ...
  "DR Congo":"RD Congo","England":"Inglaterra","Croatia":"Croácia",
  "Ghana":"Gana","Panama":"Panamá","Uzbekistan":"Uzbequistão",
...
// STATE
// ───────────────────────────────────────────
let SC = {};           // placar por id de jogo
let FD_SCORERS = [];   // artilheiros da football-data.org
let FD_STANDINGS = {}; // tabelas por grupo
let apiOk = false, fdOk = false;
...
function estMin(d, t) {
  const [h, m] = t.split(":").map(Number);
  const kick = new Date(d + "T12:00:00");
  kick.setHours(h, m, 0);
...
// ───────────────────────────────────────────
// API 1 — TheSportsDB (placares ao vivo, sem chave)
// ───────────────────────────────────────────
async function fetchTSDB() {
  try {
    const r = await fetch(`${TSDB}/eventsday.php?d=${todayStr()}&s=Soccer`, { signal: AbortSignal.timeout(7000) });
    if (!r.ok) throw 0;
    const data = await r.json();
    (data.events || [])
      .filter(e => e.strLeague && (e.strLeague.includes("World Cup") || e.strLeague.includes("FIFA")))
      .forEach(ev => {
        const match = M.find(m => FL[m.h] && nmMatch(m.h, ev.strHomeTeam) && nmMatch(m.a, ev.strAwayTeam));
        if (!match) return;
        const st = (ev.strStatus || "").toLowerCase();
        let status = "upcoming";
        if (["ft","aet","pens","ap"].includes(st)) status = "finished";
        else if (st === "ht" || /^\d+/.test(ev.strStatus) || ["live","1h","2h"].includes(st)) status = "live";
        SC[match.id] = { ...SC[match.id], hs: ev.intHomeScore, as: ev.intAwayScore, status, min: ev.strStatus };
      });
    apiOk = true;
  } catch { apiOk = false; }
}
...
// ───────────────────────────────────────────
// API 2 — football-data.org (standings, scorers, detalhes)
// ───────────────────────────────────────────
  ...
  try {
    ...
    (data.matches || []).forEach(fm => {
      ...
      if (hs != null || status !== "upcoming") {
        SC[match.id] = { ...SC[match.id], hs: hs ?? SC[match.id]?.hs, as: as ?? SC[match.id]?.as, status, min: min || SC[match.id]?.min, fdId: fm.id };
      }
    });
    fdOk = true;
  } catch { fdOk = false; }
...
async function fetchFDStandings() {
  try {
    ...
    (data.standings || []).forEach(grp => {
      const letter = (grp.group || "").replace(/.*GROUP_/, "").replace("GROUP_", "");
      FD_STANDINGS[letter] = (grp.table || []).map(row => ({
        team: row.team?.name || "",
        pts: row.points || 0, played: row.playedGames || 0,
        won: row.won || 0, drawn: row.draw || 0, lost: row.lost || 0,
        gf: row.goalsFor || 0, ga: row.goalsAgainst || 0,
        gd: row.goalDifference || 0, form: row.form || ""
      }));
    });
  } catch {}
}
...
// ───────────────────────────────────────────
// TABELA LOCAL (fallback se FD offline)
// ───────────────────────────────────────────
function calcLocalGroup(gl) {
  ...
  M.filter(m => m.g === gl && m.ph === "grupos").forEach(m => {
    const sc = SC[m.id]; const status = mSt(m);
...
// ───────────────────────────────────────────
// RENDER: PÁGINA GRUPOS
// ───────────────────────────────────────────
function renderGrupos() {
  const useFD = Object.keys(FD_STANDINGS).length > 0;
  let html = `<div style="font-family:'Barlow Condensed',sans-serif;font-size:11px;color:var(--text3);padding:2px 2px 10px">${useFD ? '<span style="color:var(--green)">✓ Dados: football-data.org</span>' : "Calculado localmente · 🔴 = parcial ao vivo"}</div>`;
...
<div class="grp-hdr" onclick="toggleGrp('${gl}')">
  ...
  ${hasLive ? '<span style="color:var(--live);font-family:\'Barlow Condensed\',sans-serif;font-size:10px;font-weight:800;animation:pulse 1.5s infinite;margin-left:4px">🔴 AO VIVO</span>' : ""}
...
<div class="st-legend">
  <span><span class="ld" style="background:var(--green)"></span>Classificado</span>
  <span><span class="ld" style="background:var(--gold)"></span>Melhor 3º</span>
  <span><span class="ld" style="background:#4B5563"></span>Eliminado</span>
  ${hasLive ? '<span><span class="ld" style="background:var(--live)"></span>Parcial ao vivo</span>' : ""}
</div>
...
async function openModal(id) {
  ...
  startModalTimer(m);
  if (mSt(m) !== "upcoming") {
    const detail = await fetchFDMatchDetail(SC[m.id]?.fdId);
    if (modalMatchId === id) {
...
function buildModalShell(m, detail) {
  ...
  else if (!detail) bodyHTML = '<div class="no-data">📡 Buscando eventos...<br><span style="font-size:10px">Disponível durante e após o jogo</span></div>';
...
function buildDetailBody(m, sc, detail) {
  ...
  if (goals.length) {
    ...
    html += '<div class="modal-sec"><div class="modal-sec-title">⚽ Gols</div><div class="no-data" style="font-size:11px">Marcadores não disponíveis via API</div></div>';
  }
  // Cartões
  if (bookings.length) {
  ...
  if (!goals.length && !bookings.length && !subs.length && !stats.length) {
    html += '<div class="no-data">📡 Detalhes não disponíveis<br><span style="font-size:10px">Dados aparecem após o apito final</span></div>';
  }
  return html;
}
...
function renderStats() {
  ...
  const lc = liveCount();
  let totalG = 0;
  M.forEach(m => { const s = SC[m.id]; if (s && s.status === "finished" && s.hs != null) totalG += +s.hs + +s.as; });
  const avg = played > 0 ? (totalG / played).toFixed(1) : "—";
  const pct = Math.round(played / 104 * 100);
  // Times: gols marcados e sofridos
  let teamStats = [];
  Object.keys(GROUPS).forEach(gl => {
  ...
  // Artilheiros da football-data.org
  let scorersH = "";
  if (FD_SCORERS.length) {
    scorersH = FD_SCORERS.slice(0, 15).map((s, i) => {
    ...
    scorersH = '<div class="no-data">Disponível após início da Copa · <span style="color:var(--gold)">football-data.org</span></div>';
  }
  const atkH = topAtk.length
    ...
    : '<div class="empty">Aguardando início da Copa</div>';
...
<div class="list-blk">
  <div class="lb-hdr"><span class="lhi">🥅</span><h3>GOLS POR SELEÇÃO</h3><span class="api-src">${fdOk?"✓ football-data.org":"local"}</span></div>
  ${atkH}
</div>
<div class="list-blk">
  <div class="lb-hdr"><span class="lhi">🛡️</span><h3>MENOS GOLS SOFRIDOS</h3><span class="api-src">${fdOk?"✓ football-data.org":"local"}</span></div>
  ${defH}
</div>
<div class="list-blk">
  ...
  <div style="padding:12px 14px;font-family:'Barlow Condensed',sans-serif;font-size:12px;color:var(--text3);line-height:1.9">
    Cartões e faltas disponíveis no <strong style="color:var(--gold)">detalhe de cada jogo</strong>.<br>
    Toque em qualquer partida encerrada para ver eventos completos.<br>
    🟨 2 amarelos acumulados = suspensão automática<br>
    🟥 Cartão vermelho = próximo jogo suspenso<br>
...
<div class="list-blk">
  ...
  <table class="info-tbl">
    ...
    <tr><td>Abertura</td><td>11 Jun 2026 · Cidade do México</td></tr>
    <tr><td>Final</td><td>19 Jul 2026 · MetLife, Nova York</td></tr>
    <tr><td>🇧🇷 Brasil — Grupo C</td><td>🇧🇷🇲🇦🇭🇹🏴󠁧󠁢󠁳󠁣󠁴󠁿</td></tr>
    <tr><td>API de dados</td><td style="color:var(--gold)">football-data.org</td></tr>
    <tr><td>Criado por</td><td style="color:var(--gold)">Pscheidt</td></tr>
  </table>
</div>
<div class="src">Stats: <a href="https://football-data.org">football-data.org</a> · Placares: <a href="https://thesportsdb.com">TheSportsDB</a></div>`;
}
...
function render() {
  ...
  document.getElementById("apiWarn").classList.toggle("on", !apiOk && !fdOk);
...
async function loadAll() {
  ...
  await Promise.all([fetchTSDB(), fetchFDMatches(), fetchFDStandings(), fetchFDScorers()]);
  render();
  const now = new Date();
  const src = fdOk ? "✓ football-data.org" : apiOk ? "✓ TheSportsDB" : "⚠ offline";
  document.getElementById("updLbl").textContent = `${src} · ${now.toLocaleTimeString("pt-BR", { hour:"2-digit", minute:"2-digit" })}`;
  btn.classList.remove("spin");
}

// Auto-refresh: 60s se ao vivo, 5min caso contrário
