...
const ESPN_CACHE_KEY_V22="copa2026_data_v5";
...
function renderStats(){
  ...
  return`<div class="stats-version">✓ V5"kpi-grid">
...
<table class="info-tbl">
  ...
  <tr><td>Versão</td><td style="color:var(--gold)">V5</td></tr>
</table></div>`;
}
...
function renderBrasil(){
  ...
  return `<div class="pro-badge">✓ V5"team-hero">
...
async function loadAll(){
  ...
  resolveKnockoutV23();
  if(typeof invalidateStatsCacheV5==="function")invalidateStatsCacheV5();
  render();
  if(typeof prewarmStatsCacheV5==="function")prewarmStatsCacheV5();
...
// ============================
// V5 - CAMADA ESTATÍSTICA NORMALIZADA
// ============================

function normMinuteV5(v){
...
function cardEventKeyV5(m,c){
  const eventId=c.id||c.eventId||c.uid||c.guid||"";
  if(eventId)return `${m.id}|id:${eventId}`;
  return [
    m.id,
    normPlayerV5(c.name||c.player||c.athlete||c.displayName||""),
    canon(c.team||""),
    cardType(c),
    normMinuteV5(c.minute||c.time||c.clock||"?")??"?"
  ].join("|");
}

function normalizeCardEventsV5(m,side){
  const team=side==="home"?m.h:m.a;
  ...
  raw.forEach(c=>{
    ...
    const type=cardType(c);
    const key=cardEventKeyV5(m,{...c,team});
    if(!map.has(key)){
      map.set(key,{
        id:c.id||c.eventId||null,
        name,
        playerKey:normPlayerV5(name),
        team,
        minute:normMinuteV5(c.minute||c.time||c.clock),
        card:type,
...
function teamOfficialCardTotalsV5(m,side){
...
function teamCardsForMatchV5(m,side){
  const events=normalizeCardEventsV5(m,side);
  const official=teamOfficialCardTotalsV5(m,side);
  ...
  // Regra V5: se houver total oficial, ele é soberano.
...
function tournamentTeamCardsV5(team){
  let yellow=0,red=0;
  const players=new Map();

  F.filter(m=>mSt(m)!=="upcoming"&&(nm(m.h,team)||nm(m.a,team))).forEach(m=>{
    const side=nm(m.h,team)?"home":"away";
    const pack=teamCardsForMatchV5(m,side);
    yellow+=pack.yellow;
    red+=pack.red;
...
function goalsByPlayerForTeamV5(team){
  const map=new Map();
  const seen=new Set();
    ...
    (allGoalsForMatchV9(m,ofb)||[])
      ...
      .forEach(g=>{
        ...
        const key=[
          ...
          normPlayerV5(name),
          canon(team),
          normMinuteV5(g.minute||g.time||g.clock)||"?"
        ].join("|");
        if(seen.has(key))return;
        seen.add(key);

        const pkey=normPlayerV5(name);
...
function aggregateTeamStatsV5(team){
  const out={
...
function v3Card(title,value,sub,details,icon="📊"){
  return `<div class="v3-card" onclick="toggleV5Card(this)">
    <div class="v3-head">
      <div>
...
function rowsV5(items){
  return items.map(x=>`<div class="v3-row"><span>${x[0]}</span><strong>${x[1]}</strong></div>`).join("");
}

function renderBrasilV5(){
  const s=aggregateTeamStatsV5("Brazil");
  const cards=tournamentTeamCardsV5("Brazil");
  const scorers=goalsByPlayerForTeamV5("Brazil");
  ...
  const scorersRows=scorers.length
    ? rowsV5(scorers.map(x=>[x.name,`${x.goals} gol${x.goals===1?"":"s"}`]))
    : '<div class="v3-muted">Sem gols identificados</div>';

  const discRows=discipline.length
    ? rowsV5(discipline.map(x=>[x.name,`${x.yellow} 🟨 · ${x.red} 🟥`]))
  ...
  return `
    ...
    <div class="v3-grid">
      ${v3Card("Jogos",s.games,`${s.wins}V · ${s.draws}E · ${s.losses}D`,
        rowsV5([["Aproveitamento",`${s.performance.toFixed(0)}%`],["Pontos",s.points],["Saldo",s.gd>=0?`+${s.gd}`:s.gd]]),"🏟️")}
      ...
      ${v3Card("Defesa",s.ga,`${s.cleanSheets} jogo${s.cleanSheets===1?"":"s"} sem sofrer`,
        rowsV5([["Gols sofridos",s.ga],["Saldo",s.gd],["Clean sheets",s.cleanSheets]]),"🛡️")}
      ${v3Card("Chutes",s.shots,s.shotAccuracy!==null?`${s.shotAccuracy.toFixed(1)}% no alvo`:"dados parciais",
        rowsV5([["Chutes",s.shots],["No alvo",s.shotsOnGoal],["Conversão",s.conversion!==null?`${s.conversion.toFixed(1)}%`:"—"]]),"🎯")}
      ${v3Card("Posse média",s.possessionAvg!==null?`${s.possessionAvg.toFixed(1)}%`:"—","média no torneio",
        rowsV5([["Posse média",s.possessionAvg!==null?`${s.possessionAvg.toFixed(1)}%`:"—"],["Precisão de passes",s.passAccuracyAvg!==null?`${s.passAccuracyAvg.toFixed(1)}%`:"—"],["Passes",s.passes||"—"]]),"🧠")}
      ${v3Card("Escanteios",s.corners,"total no torneio",
        rowsV5([["Escanteios",s.corners],["Faltas",s.fouls],["Impedimentos",s.offsides]]),"🚩")}
      ${v3Card("Disciplina",cards.yellow,`${cards.red} expulsão${cards.red===1?"":"ões"}`,
        rowsV5([["Amarelos",cards.yellow],["Vermelhos",cards.red],["Média/jogo",s.games?`${(cards.total/s.games).toFixed(2)}`:"—"]]),"⚖️")}
    </div>
...
function renderStatsV5(){
  ...
  const teamRows=teams.map(team=>{
    const st=aggregateTeamStatsV5(team);
    const cd=tournamentTeamCardsV5(team);
    return {team,...st,yellow:cd.yellow,red:cd.red,totalCards:cd.total};
  });

  const scorersMap=new Map();
  teams.forEach(team=>{
    goalsByPlayerForTeamV5(team).forEach(p=>{
      const k=normPlayerV5(p.name)+"|"+canon(team);
  ...
  return `
    ...
    <div class="v3-grid">
      ...
      ${v3Card("Maior posse",topPoss[0]?.possessionAvg!==null&&topPoss[0]?`${topPoss[0].possessionAvg.toFixed(1)}%`:"—",topPoss[0]?pt(topPoss[0].team):"dados parciais",
        rowsV5(topPoss.map(x=>[`${fl(x.team)} ${pt(x.team)}`,`${x.possessionAvg.toFixed(1)}%`])),"🧠")}
      ${v3Card("Mais chutes",Math.max(...teamRows.map(x=>x.shots),0),
        ...
        rowsV5([...teamRows].sort((a,b)=>b.shots-a.shots).slice(0,10).map(x=>[`${fl(x.team)} ${pt(x.team)}`,x.shots])),"🎯")}
      ${v3Card("Mais faltas",Math.max(...teamRows.map(x=>x.fouls),0),
        ...
        rowsV5([...teamRows].sort((a,b)=>b.fouls-a.fouls).slice(0,10).map(x=>[`${fl(x.team)} ${pt(x.team)}`,x.fouls])),"⚖️")}
    </div>
...
renderBrasil = renderBrasilV5;
renderStats = renderStatsV5;


// ============================
// V5 - ESTATÍSTICAS RÁPIDAS E CORRIGIDAS
// ============================

let STATS_CACHE_V5={key:"",model:null,building:false};

function invalidateStatsCacheV5(){
  STATS_CACHE_V5={key:"",model:null,building:false};
}

function statsFingerprintV5(){
...
function statSideV5(stats,key,side){
  const map={
    shotsOnGoal:"shotsOnTarget",
    passes:"totalPasses"
  };
  const sourceKey=map[key]||key;
  return numberV5(stats?.[sourceKey]?.[side]);
}

function simpleCardEventsV5(m,side){
  const team=side==="home"?m.h:m.a;
  const raw=[];
  const ofb=ofbMatch(m.h,m.a);
  ...
  raw.forEach(c=>{
    ...
    const playerKey=normPlayerV5(name);
    const type=cardType(c);
    const minute=normMinuteV5(c.minute||c.time||c.clock);
...
function buildStatsModelV5(){
  ...
  const byTeam=new Map(teams.map(t=>[t,emptyTeamV5(t)]));
  ...
  const getTeam=name=>{
    ...
    if(!exact&&name&&!isPlaceholderTeamV23(name)){
      exact=name;
      teams.push(name);
      byTeam.set(name,emptyTeamV5(name));
    }
  ...
  F.filter(m=>mSt(m)==="finished"&&!isPlaceholderTeamV23(m.h)&&!isPlaceholderTeamV23(m.a))
   .forEach(m=>{
    ...
    [["home",home],["away",away]].forEach(([side,t])=>{
      ...
      const values={
        shots:statSideV5(stats,"shots",side),
        shotsOnGoal:statSideV5(stats,"shotsOnGoal",side),
        corners:statSideV5(stats,"corners",side),
        fouls:statSideV5(stats,"fouls",side),
        offsides:statSideV5(stats,"offsides",side),
        passes:statSideV5(stats,"passes",side),
        possession:statSideV5(stats,"possession",side),
        passAccuracy:statSideV5(stats,"passAccuracy",side)
      };
    ...
    (allGoalsForMatchV9(m,ofb)||[])
      ...
      .forEach(g=>{
        ...
        const playerKey=normPlayerV5(name)+"|"+canon(team);
...
function getStatsModelV5(){
  const key=statsFingerprintV5();
  if(STATS_CACHE_V5.model&&STATS_CACHE_V5.key===key){
    return STATS_CACHE_V5.model;
  }

  const model=buildStatsModelV5();
  STATS_CACHE_V5={key,model,building:false};
  return model;
}

function prewarmStatsCacheV5(){
  setTimeout(()=>{
    try{getStatsModelV5()}catch(e){console.warn("V5 stats cache",e)}
  },80);
}

function teamModelV5(team){
  const model=getStatsModelV5();
  return model.teams.find(x=>nm(x.team,team))||emptyTeamV5(team);
}

function valueOrDashV5(v,suffix=""){
  return v===null||v===undefined||Number.isNaN(v)?"—":`${v}${suffix}`;
}

function renderBrasilFastV5(){
  const s=teamModelV5("Brazil");
  const model=getStatsModelV5();
  ...
  return `
    ...
    <div class="v3-grid">
      ...
      ${v3Card("Defesa",s.ga,`${s.cleanSheets} jogo${s.cleanSheets===1?"":"s"} sem sofrer`,
        rowsV5([["Gols sofridos",s.ga],["Saldo",s.gd],["Jogos sem sofrer",s.cleanSheets]]),"🛡️")}

      ${v3Card("Chutes",s.statsGames?valueOrDashV5(s.shots):"—",
        s.statsGames&&s.shotAccuracy!==null?`${s.shotAccuracy.toFixed(1)}% no alvo`:"dado ainda não disponível",
        rowsV5([["Chutes",s.statsGames?s.shots:"—"],["No alvo",s.statsGames?s.shotsOnGoal:"—"],["Conversão",s.conversion!==null?`${s.conversion.toFixed(1)}%`:"—"]]),"🎯")}
      ...
      ${v3Card("Posse média",s.possessionAvg!==null?`${s.possessionAvg.toFixed(1)}%`:"—",
        ...
        rowsV5([["Posse média",s.possessionAvg!==null?`${s.possessionAvg.toFixed(1)}%`:"—"],["Precisão de passes",s.passAccuracyAvg!==null?`${s.passAccuracyAvg.toFixed(1)}%`:"—"],["Passes",s.statsGames?s.passes:"—"]]),"🧠")}

      ${v3Card("Escanteios",s.statsGames?s.corners:"—",
        s.statsGames?"total no torneio":"dado ainda não disponível",
        rowsV5([["Escanteios",s.statsGames?s.corners:"—"],["Faltas",s.statsGames?s.fouls:"—"],["Impedimentos",s.statsGames?s.offsides:"—"]]),"🚩")}

      ${v3Card("Disciplina",s.yellow,`${s.red} expulsão${s.red===1?"":"ões"}`,
        rowsV5([["Amarelos",s.yellow],["Vermelhos",s.red],["Média por jogo",s.games?(s.totalCards/s.games).toFixed(2):"—"]]),"⚖️")}
    </div>
  ...
  return `
    ...
    <div class="v3-grid">
      ...
      ${v3Card("Artilheiros",model.scorers[0]?.goals||"—",
        ...
        rowsV5(model.scorers.slice(0,20).map(x=>[`${x.name} · ${pt(x.team)}`,x.goals])),"🥇")}

      ${v3Card("Cartões",totalYellow+totalRed,
        `${totalYellow} amarelos · ${totalRed} vermelhos`,
        topCards.length?rowsV5(topCards.map(x=>[`${fl(x.team)} ${pt(x.team)}`,`${x.yellow} 🟨 · ${x.red} 🟥`])):'<div class="v3-muted">Dados ainda não disponíveis</div>',"🟨")}
      ...
      ${v3Card("Maior posse",topPoss[0]?`${topPoss[0].possessionAvg.toFixed(1)}%`:"—",
        ...
        topPoss.length?rowsV5(topPoss.map(x=>[`${fl(x.team)} ${pt(x.team)}`,`${x.possessionAvg.toFixed(1)}%`])):'<div class="v3-muted">A fonte ainda não entregou posse consolidada</div>',"🧠")}

      ${v3Card("Mais chutes",topShots[0]?.shots??"—",
        topShots[0]?pt(topShots[0].team):"dado ainda não disponível",
        topShots.length?rowsV5(topShots.map(x=>[`${fl(x.team)} ${pt(x.team)}`,x.shots])):'<div class="v3-muted">A fonte ainda não entregou chutes consolidados</div>',"🎯")}

      ${v3Card("Mais faltas",topFouls[0]?.fouls??"—",
        topFouls[0]?pt(topFouls[0].team):"dado ainda não disponível",
        topFouls.length?rowsV5(topFouls.map(x=>[`${fl(x.team)} ${pt(x.team)}`,x.fouls])):'<div class="v3-muted">A fonte ainda não entregou faltas consolidadas</div>',"⚖️")}
    </div>
...
renderBrasil=renderBrasilFastV5;
renderStats=renderStatsFastV5;

console.log('Copa 2026 V5 carregado');
