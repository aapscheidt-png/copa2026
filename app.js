...
const ESPN_CACHE_KEY_V22="copa2026_data_v7";
...
function manualLiveV7(m){
  const x = LIVE_MATCHES[matchKey(m.h,m.a)];
  if(!x) return null;
  const clk = liveClockV7(x.startISO);
  return {
    hs:x.hs,
    as:x.as,
    hasScore:true,
...
function renderStats(){
  ...
  return`<div class="stats-version">✓ V7"kpi-grid">
...
<table class="info-tbl">
  ...
  <tr><td>Versão</td><td style="color:var(--gold)">V7</td></tr>
</table></div>`;
}
...
function renderBrasil(){
  ...
  return `<div class="pro-badge">✓ V7"team-hero">
...
async function loadAll(){
  ...
  await Promise.allSettled([
    safeRunV7(fetchStaticESPN,18000),
    safeRunV7(fetchWCGames,3500),
    safeRunV7(fetchWCGroups,3500),
    safeRunV7(fetchWCScorers,3500),
    safeRunV7(fetchOFB,3500),
    safeRunV7(fetchFD,3500)
  ]);

  resolveKnockoutV23();
  if(typeof invalidateStatsCacheV7==="function")invalidateStatsCacheV7();
  render();
  if(typeof prewarmStatsCacheV7==="function")prewarmStatsCacheV7();
...
// ============================
// V7 - CAMADA ESTATÍSTICA NORMALIZADA
// ============================

function normMinuteV7(v){
...
function cardEventKeyV7(m,c){
  const eventId=c.id||c.eventId||c.uid||c.guid||"";
  if(eventId)return `${m.id}|id:${eventId}`;
  return [
    m.id,
    normPlayerV7(c.name||c.player||c.athlete||c.displayName||""),
    canon(c.team||""),
    cardType(c),
    normMinuteV7(c.minute||c.time||c.clock||"?")??"?"
  ].join("|");
}

function normalizeCardEventsV7(m,side){
  const team=side==="home"?m.h:m.a;
  ...
  raw.forEach(c=>{
    ...
    const type=cardType(c);
    const key=cardEventKeyV7(m,{...c,team});
    if(!map.has(key)){
      map.set(key,{
        id:c.id||c.eventId||null,
        name,
        playerKey:normPlayerV7(name),
        team,
        minute:normMinuteV7(c.minute||c.time||c.clock),
        card:type,
...
function teamOfficialCardTotalsV7(m,side){
...
function teamCardsForMatchV7(m,side){
  const events=normalizeCardEventsV7(m,side);
  const official=teamOfficialCardTotalsV7(m,side);
  ...
  // Regra V7: se houver total oficial, ele é soberano.
...
function tournamentTeamCardsV7(team){
  let yellow=0,red=0;
  const players=new Map();

  F.filter(m=>mSt(m)!=="upcoming"&&(nm(m.h,team)||nm(m.a,team))).forEach(m=>{
    const side=nm(m.h,team)?"home":"away";
    const pack=teamCardsForMatchV7(m,side);
    yellow+=pack.yellow;
    red+=pack.red;
...
function goalsByPlayerForTeamV7(team){
  const map=new Map();
  const seen=new Set();
    ...
    (allGoalsForMatchV9(m,ofb)||[])
      ...
      .forEach(g=>{
        ...
        const key=[
          ...
          normPlayerV7(name),
          canon(team),
          normMinuteV7(g.minute||g.time||g.clock)||"?"
        ].join("|");
        if(seen.has(key))return;
        seen.add(key);

        const pkey=normPlayerV7(name);
...
function aggregateTeamStatsV7(team){
  const out={
...
function v3Card(title,value,sub,details,icon="📊"){
  return `<div class="v3-card" onclick="toggleV7Card(this)">
    <div class="v3-head">
      <div>
...
function rowsV7(items){
  return items.map(x=>`<div class="v3-row"><span>${x[0]}</span><strong>${x[1]}</strong></div>`).join("");
}

function renderBrasilV7(){
  const s=aggregateTeamStatsV7("Brazil");
  const cards=tournamentTeamCardsV7("Brazil");
  const scorers=goalsByPlayerForTeamV7("Brazil");
  ...
  const scorersRows=scorers.length
    ? rowsV7(scorers.map(x=>[x.name,`${x.goals} gol${x.goals===1?"":"s"}`]))
    : '<div class="v3-muted">Sem gols identificados</div>';

  const discRows=discipline.length
    ? rowsV7(discipline.map(x=>[x.name,`${x.yellow} 🟨 · ${x.red} 🟥`]))
  ...
  return `
    ...
    <div class="v3-grid">
      ${v3Card("Jogos",s.games,`${s.wins}V · ${s.draws}E · ${s.losses}D`,
        rowsV7([["Aproveitamento",`${s.performance.toFixed(0)}%`],["Pontos",s.points],["Saldo",s.gd>=0?`+${s.gd}`:s.gd]]),"🏟️")}
      ...
      ${v3Card("Defesa",s.ga,`${s.cleanSheets} jogo${s.cleanSheets===1?"":"s"} sem sofrer`,
        rowsV7([["Gols sofridos",s.ga],["Saldo",s.gd],["Clean sheets",s.cleanSheets]]),"🛡️")}
      ${v3Card("Chutes",s.shots,s.shotAccuracy!==null?`${s.shotAccuracy.toFixed(1)}% no alvo`:"dados parciais",
        rowsV7([["Chutes",s.shots],["No alvo",s.shotsOnGoal],["Conversão",s.conversion!==null?`${s.conversion.toFixed(1)}%`:"—"]]),"🎯")}
      ${v3Card("Posse média",s.possessionAvg!==null?`${s.possessionAvg.toFixed(1)}%`:"—","média no torneio",
        rowsV7([["Posse média",s.possessionAvg!==null?`${s.possessionAvg.toFixed(1)}%`:"—"],["Precisão de passes",s.passAccuracyAvg!==null?`${s.passAccuracyAvg.toFixed(1)}%`:"—"],["Passes",s.passes||"—"]]),"🧠")}
      ${v3Card("Escanteios",s.corners,"total no torneio",
        rowsV7([["Escanteios",s.corners],["Faltas",s.fouls],["Impedimentos",s.offsides]]),"🚩")}
      ${v3Card("Disciplina",cards.yellow,`${cards.red} expulsão${cards.red===1?"":"ões"}`,
        rowsV7([["Amarelos",cards.yellow],["Vermelhos",cards.red],["Média/jogo",s.games?`${(cards.total/s.games).toFixed(2)}`:"—"]]),"⚖️")}
    </div>
...
function renderStatsV7(){
  ...
  const teamRows=teams.map(team=>{
    const st=aggregateTeamStatsV7(team);
    const cd=tournamentTeamCardsV7(team);
    return {team,...st,yellow:cd.yellow,red:cd.red,totalCards:cd.total};
  });

  const scorersMap=new Map();
  teams.forEach(team=>{
    goalsByPlayerForTeamV7(team).forEach(p=>{
      const k=normPlayerV7(p.name)+"|"+canon(team);
  ...
  return `
    ...
    <div class="v3-grid">
      ...
      ${v3Card("Maior posse",topPoss[0]?.possessionAvg!==null&&topPoss[0]?`${topPoss[0].possessionAvg.toFixed(1)}%`:"—",topPoss[0]?pt(topPoss[0].team):"dados parciais",
        rowsV7(topPoss.map(x=>[`${fl(x.team)} ${pt(x.team)}`,`${x.possessionAvg.toFixed(1)}%`])),"🧠")}
      ${v3Card("Mais chutes",Math.max(...teamRows.map(x=>x.shots),0),
        ...
        rowsV7([...teamRows].sort((a,b)=>b.shots-a.shots).slice(0,10).map(x=>[`${fl(x.team)} ${pt(x.team)}`,x.shots])),"🎯")}
      ${v3Card("Mais faltas",Math.max(...teamRows.map(x=>x.fouls),0),
        ...
        rowsV7([...teamRows].sort((a,b)=>b.fouls-a.fouls).slice(0,10).map(x=>[`${fl(x.team)} ${pt(x.team)}`,x.fouls])),"⚖️")}
    </div>
...
renderBrasil = renderBrasilV7;
renderStats = renderStatsV7;


// ============================
// V7 - ESTATÍSTICAS RÁPIDAS E CORRIGIDAS
// ============================

let STATS_CACHE_V7={key:"",model:null,building:false};

function invalidateStatsCacheV7(){
  STATS_CACHE_V7={key:"",model:null,building:false};
}

function statsFingerprintV7(){
...
function statSideV7(stats,key,side){
  const map={
    shotsOnGoal:"shotsOnTarget",
    passes:"totalPasses"
  };
  const sourceKey=map[key]||key;
  return numberV7(stats?.[sourceKey]?.[side]);
}

function simpleCardEventsV7(m,side){
  const team=side==="home"?m.h:m.a;
  const raw=[];
  const ofb=ofbMatch(m.h,m.a);
  ...
  raw.forEach(c=>{
    ...
    const playerKey=normPlayerV7(name);
    const type=cardType(c);
    const minute=normMinuteV7(c.minute||c.time||c.clock);
...
function buildStatsModelV7(){
  ...
  const byTeam=new Map(teams.map(t=>[t,emptyTeamV7(t)]));
  ...
  const getTeam=name=>{
    ...
    if(!exact&&name&&!isPlaceholderTeamV23(name)){
      exact=name;
      teams.push(name);
      byTeam.set(name,emptyTeamV7(name));
    }
  ...
  F.filter(m=>mSt(m)==="finished"&&!isPlaceholderTeamV23(m.h)&&!isPlaceholderTeamV23(m.a))
   .forEach(m=>{
    ...
    [["home",home],["away",away]].forEach(([side,t])=>{
      ...
      const values={
        shots:statSideV7(stats,"shots",side),
        shotsOnGoal:statSideV7(stats,"shotsOnGoal",side),
        corners:statSideV7(stats,"corners",side),
        fouls:statSideV7(stats,"fouls",side),
        offsides:statSideV7(stats,"offsides",side),
        passes:statSideV7(stats,"passes",side),
        possession:statSideV7(stats,"possession",side),
        passAccuracy:statSideV7(stats,"passAccuracy",side)
      };
    ...
    (allGoalsForMatchV9(m,ofb)||[])
      ...
      .forEach(g=>{
        ...
        const playerKey=normPlayerV7(name)+"|"+canon(team);
...
function getStatsModelV7(){
  const key=statsFingerprintV7();
  if(STATS_CACHE_V7.model&&STATS_CACHE_V7.key===key){
    return STATS_CACHE_V7.model;
  }

  const model=buildStatsModelV7();
  STATS_CACHE_V7={key,model,building:false};
  return model;
}

function prewarmStatsCacheV7(){
  setTimeout(()=>{
    try{getStatsModelV7()}catch(e){console.warn("V7 stats cache",e)}
  },80);
}

function teamModelV7(team){
  const model=getStatsModelV7();
  return model.teams.find(x=>nm(x.team,team))||emptyTeamV7(team);
}

function valueOrDashV7(v,suffix=""){
  return v===null||v===undefined||Number.isNaN(v)?"—":`${v}${suffix}`;
}

function renderBrasilFastV7(){
  const s=teamModelV7("Brazil");
  const model=getStatsModelV7();
  ...
  return `
    ...
    <div class="v3-grid">
      ...
      ${v3Card("Defesa",s.ga,`${s.cleanSheets} jogo${s.cleanSheets===1?"":"s"} sem sofrer`,
        rowsV7([["Gols sofridos",s.ga],["Saldo",s.gd],["Jogos sem sofrer",s.cleanSheets]]),"🛡️")}

      ${v3Card("Chutes",s.statsGames?valueOrDashV7(s.shots):"—",
        s.statsGames&&s.shotAccuracy!==null?`${s.shotAccuracy.toFixed(1)}% no alvo`:"dado ainda não disponível",
        rowsV7([["Chutes",s.statsGames?s.shots:"—"],["No alvo",s.statsGames?s.shotsOnGoal:"—"],["Conversão",s.conversion!==null?`${s.conversion.toFixed(1)}%`:"—"]]),"🎯")}
      ...
      ${v3Card("Posse média",s.possessionAvg!==null?`${s.possessionAvg.toFixed(1)}%`:"—",
        ...
        rowsV7([["Posse média",s.possessionAvg!==null?`${s.possessionAvg.toFixed(1)}%`:"—"],["Precisão de passes",s.passAccuracyAvg!==null?`${s.passAccuracyAvg.toFixed(1)}%`:"—"],["Passes",s.statsGames?s.passes:"—"]]),"🧠")}

      ${v3Card("Escanteios",s.statsGames?s.corners:"—",
        s.statsGames?"total no torneio":"dado ainda não disponível",
        rowsV7([["Escanteios",s.statsGames?s.corners:"—"],["Faltas",s.statsGames?s.fouls:"—"],["Impedimentos",s.statsGames?s.offsides:"—"]]),"🚩")}

      ${v3Card("Disciplina",s.yellow,`${s.red} expulsão${s.red===1?"":"ões"}`,
        rowsV7([["Amarelos",s.yellow],["Vermelhos",s.red],["Média por jogo",s.games?(s.totalCards/s.games).toFixed(2):"—"]]),"⚖️")}
    </div>
  ...
  return `
    ...
    <div class="v3-grid">
      ...
      ${v3Card("Artilheiros",model.scorers[0]?.goals||"—",
        ...
        rowsV7(model.scorers.slice(0,20).map(x=>[`${x.name} · ${pt(x.team)}`,x.goals])),"🥇")}

      ${v3Card("Cartões",totalYellow+totalRed,
        `${totalYellow} amarelos · ${totalRed} vermelhos`,
        topCards.length?rowsV7(topCards.map(x=>[`${fl(x.team)} ${pt(x.team)}`,`${x.yellow} 🟨 · ${x.red} 🟥`])):'<div class="v3-muted">Dados ainda não disponíveis</div>',"🟨")}
      ...
      ${v3Card("Maior posse",topPoss[0]?`${topPoss[0].possessionAvg.toFixed(1)}%`:"—",
        ...
        topPoss.length?rowsV7(topPoss.map(x=>[`${fl(x.team)} ${pt(x.team)}`,`${x.possessionAvg.toFixed(1)}%`])):'<div class="v3-muted">A fonte ainda não entregou posse consolidada</div>',"🧠")}

      ${v3Card("Mais chutes",topShots[0]?.shots??"—",
        topShots[0]?pt(topShots[0].team):"dado ainda não disponível",
        topShots.length?rowsV7(topShots.map(x=>[`${fl(x.team)} ${pt(x.team)}`,x.shots])):'<div class="v3-muted">A fonte ainda não entregou chutes consolidados</div>',"🎯")}

      ${v3Card("Mais faltas",topFouls[0]?.fouls??"—",
        topFouls[0]?pt(topFouls[0].team):"dado ainda não disponível",
        topFouls.length?rowsV7(topFouls.map(x=>[`${fl(x.team)} ${pt(x.team)}`,x.fouls])):'<div class="v3-muted">A fonte ainda não entregou faltas consolidadas</div>',"⚖️")}
    </div>
...
renderBrasil=renderBrasilFastV7;
renderStats=renderStatsFastV7;


// ============================
// V7 - MELHORES TERCEIROS NO MATA-MATA
// ============================

function groupThirdsV7(){
  const out=[];
...
function bestThirdTeamsV7(){
  return groupThirdsV7().slice(0,8);
}

function isBestThirdSlotV7(slot){
  return /3º colocado|melhor\s*3/i.test(String(slot||""));
}

function applyBestThirdsV7(){
  const thirds=bestThirdTeamsV7();
  ...
  F.filter(m=>/^e\d+$/i.test(m.id)).forEach(m=>{
    [["h","slotH"],["a","slotA"]].forEach(([side,slotKey])=>{
      ...
      if(!isBestThirdSlotV7(slot))return;
      if(!isPlaceholderTeamV23(m[side]) && !isBestThirdSlotV7(m[side]))return;
...
const resolveKnockoutBaseV7 = resolveKnockoutV23;
resolveKnockoutV23 = function(){
  resolveKnockoutBaseV7();
  applyBestThirdsV7();
  applyOfficialParticipantsV23();
...
// ============================
// V7 - PLACAR COM PRORROGAÇÃO E PÊNALTIS
// ============================
...
function penaltyInfoV7(m){
  ...
  const hp=readPenaltyScoreV7(home);
  const ap=readPenaltyScoreV7(away);
...
function statusIsPenaltyV7(m,data){
  const txt=[
    data?.min,
    espnStatusFor(m)?.min,
...
const winnerLoserBaseV7 = winnerLoserV23;
winnerLoserV23 = function(m){
  const base=winnerLoserBaseV7(m);
  if(base)return base;

  const pk=penaltyInfoV7(m);
  if(pk)return {winner:pk.winner,loser:pk.loser};

  return null;
};

const mkCardBaseV7 = mkCard;
mkCard = function(m){
  const data=mData(m);
  ...
  let pill="";
  if(st==="live"){
    const pen=statusIsPenaltyV7(m,data);
  ...
  return `<div class="mc ${st}${isBR?" br":""}" onclick="openModal('${m.id}')">
    ...
    ${timerH}
    ${goalsSum}
    ${pk?`<div class="mc-pk-note">${pk.label}</div>`:""}
    <div class="mc-venue">${m.v}</div>
    ${st!=="upcoming"?'<div class="tap-hint">Toque para detalhes ↑</div>':""}
  </div>`;
};

console.log('Copa 2026 V7 carregado');
