...
// ── Resultados conhecidos (atualizados manualmente como fallback) ──
// Formato: "matchId": { hs, as, status:"finished" }
const KNOWN = {
  "a1": {hs:0,as:1,status:"finished"},  // Mexico 0-1 Africa do Sul
  "a2": {hs:3,as:1,status:"finished"},  // Coreia do Sul 3-1 Tchéquia
  "b1": {hs:1,as:1,status:"finished"},  // Canada 1-1 Bosnia
  "d1": {hs:2,as:0,status:"finished"},  // EUA 2-0 Paraguay
  "d2": {hs:1,as:2,status:"finished"},  // Australia 1-2 Turquia
  "b2": {hs:0,as:1,status:"finished"},  // Qatar 0-1 Switzerland (em curso)
};
...
function mSt(m) {
  const s = SC[m.id]; if (s) return s.status;
...
function getSC(m) {
  return SC[m.id] || KNOWN[m.id] || null;
}
function getMin(m) {
  const sc = SC[m.id];
...
async function fetchFDMatches() {
  try {
    const r = await fetch(`${FD}/competitions/WC/matches?season=2026`, {
      headers: FDH, signal: AbortSignal.timeout(9000)
    });
    ...
    console.warn("FD matches:", e.message);
    fdOk = false;
  }
}
...
async function fetchFDStandings() {
  try {
    const r = await fetch(`${FD}/competitions/WC/standings?season=2026`, {
      headers: FDH, signal: AbortSignal.timeout(9000)
    });
    if (!r.ok) throw new Error(`HTTP ${r.status}`);
    const data = await r.json();
    ...
    (data.standings || []).forEach(grp => {
      ...
      FD_STANDINGS[letter] = (grp.table || []).map(row => ({
        team: row.team?.name || "", pts: row.points || 0,
        played: row.playedGames || 0, won: row.won || 0,
        drawn: row.draw || 0, lost: row.lost || 0,
...
async function fetchFDScorers() {
  try {
    const r = await fetch(`${FD}/competitions/WC/scorers?season=2026&limit=20`, {
      headers: FDH, signal: AbortSignal.timeout(9000)
    });
    if (!r.ok) throw new Error(`HTTP ${r.status}`);
    const data = await r.json();
...
async function fetchFDMatchDetail(fdId) {
  if (!fdId) return null;
  try {
    const r = await fetch(`${FD}/matches/${fdId}`, {
      headers: FDH, signal: AbortSignal.timeout(8000)
    });
    if (!r.ok) throw new Error(`HTTP ${r.status}`);
    return await r.json();
...
<div class="grp-matches" id="gm-${gl}" style="display:none"><div class="gmt">JOGOS DO GRUPO ${gl}</div>`;
    gmatches.forEach(m => {
      const sc = getSC(m); const status = mSt(m);
      let sStr = m.t;
...
function buildModalShell(m, detail) {
  ...
  else if (!detail)          bodyHTML = '<div class="no-data">📡 Detalhes não disponíveis via API<br><span style="font-size:10px">football-data.org · Dados pós-apito</span></div>';
...
function buildDetailBody(m, sc, detail) {
  ...
  if (!goals.length && !bookings.length && !subs.length && !stats.length) {
    html += '<div class="no-data">📡 Detalhes indisponíveis<br><span style="font-size:10px">football-data.org · Dados aparecem após o apito final</span></div>';
  }
  return html;
}
...
function renderStats() {
  ...
  let scorersH = FD_SCORERS.length
    ...
    : '<div class="no-data">Disponível após início da Copa · <span style="color:var(--gold)">football-data.org</span></div>';
...
<div class="list-blk">
  ...
  <div style="padding:12px 14px;font-family:'Barlow Condensed',sans-serif;font-size:12px;color:var(--text3);line-height:1.9">
    Cartões disponíveis no <strong style="color:var(--gold)">detalhe de cada jogo</strong> (toque em qualquer partida).<br>
...
async function loadAll() {
  ...
  render();
  const now = new Date();
  const src = fdOk ? "✓ football-data.org" : "⚠ API offline · resultados locais";
