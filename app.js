...
// ============================
// V3 FIXO - MATA-MATA OFICIAL SEM RENOMEAR FUNÇÕES INTERNAS
// Base estável V8. Visual V3.
// ============================

const OFFICIAL_KO_FIXED_V3 = {
...
function applyOfficialKOSnapshotFixedV3(){
  Object.entries(OFFICIAL_KO_FIXED_V3).forEach(([id,o])=>{
...
function officialKODataFixedV3(m){
  const o=OFFICIAL_KO_FIXED_V3[m.id];
  if(!o)return null;

  if(o.st==="finished"){
    return {hs:o.hs,as:o.as,hasScore:true,st:"finished",min:"",pct:100,source:"fallback oficial"};
  }

  const kick=new Date(`${o.d}T${o.t}:00-03:00`).getTime();
  if(Date.now() < kick + 150*60*1000){
    return {hs:null,as:null,hasScore:false,st:"upcoming",min:"",pct:0,source:"fallback oficial"};
  }

  return null;
}

function goalKeyFixedV3(g,m){
  ...
  return `${m.id}|${side}|${name}|${min}`;
}

function dedupeGoalsFixedV3(goals,m){
  const seen=new Set();
  const out=[];
  (goals||[]).forEach(g=>{
    const key=goalKeyFixedV3(g,m);
    if(seen.has(key))return;
    seen.add(key);
    out.push(g);
  });

  const d=officialKODataFixedV3(m)||mData(m);
  if(d&&d.hasScore){
...
const mDataBeforeFixedV3 = mData;
mData = function(m){
  const official=officialKODataFixedV3(m);
  if(official)return official;
  return mDataBeforeFixedV3(m);
};

const resolveKnockoutBeforeFixedV3 = resolveKnockoutV23;
resolveKnockoutV23 = function(){
  resolveKnockoutBeforeFixedV3();
  applyOfficialKOSnapshotFixedV3();
};

const renderBeforeFixedV3 = render;
render = function(){
  applyOfficialKOSnapshotFixedV3();
  return renderBeforeFixedV3();
};

const allGoalsBeforeFixedV3 = allGoalsForMatchV9;
allGoalsForMatchV9 = function(m,ofb){
  return dedupeGoalsFixedV3(allGoalsBeforeFixedV3(m,ofb),m);
};

const goalsSummaryBeforeFixedV3 = goalsSummaryV9;
goalsSummaryV9 = function(m){
  const o=OFFICIAL_KO_FIXED_V3[m.id];
  if(o&&o.st==="finished"){
    const goals=dedupeGoalsFixedV3(allGoalsBeforeFixedV3(m,ofbMatch(m.h,m.a)),m);
    if(!goals.length)return "";
    const h=goals.filter(g=>g.side==="home").map(g=>`${fl(g.team)} ${g.name}${g.minute?` ${g.minute}'`:""}`);
    const a=goals.filter(g=>g.side==="away").map(g=>`${fl(g.team)} ${g.name}${g.minute?` ${g.minute}'`:""}`);
    return `<div class="goal-line">${[...h,...a].join(" | ")}</div>`;
  }
  return goalsSummaryBeforeFixedV3(m);
};

const penaltyInfoBeforeFixedV3 = penaltyInfoV8;
penaltyInfoV8 = function(m){
  ...
  return penaltyInfoBeforeFixedV3(m);
};
